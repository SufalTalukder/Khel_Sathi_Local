...
@section( 'content' )
				...
				<style>
					...
					<div class="pageheader" id="menu-margin">
						<div class="row">
							<div class="col-md-8">
								<h4 class="mb-0">Return Order Invoice Details</h4>
							</div>
							<div class="col-md-4">
										...
										<div class="col-md-4">
											<div class="form-group">
												<label for="challan_no">Challan No. <span class="text-danger">*</span></label>
													<select  id="challan_no" class="form-control"  name="challan_no"  required>
														
													</select>
												<div class="invalid-feedback">
													Please provide Challan No.
												</div>
												@if ($errors->has('challan_no'))
												<span class="error_mess">{{ $errors->first('challan_no') }}</span> @endif
											</div>
										</div>
		...
		$(document.body).on('change', '#order_id', function () {
			...
			$.ajax({
				...
				success: function (response) {
					...
					if(response){
						response.forEach((item)=>{
								option +=`<option  value="${item.challan_no}" >${item.challan_no}</option>`;
						});
					}
					$('#challan_no').empty();
					$('#challan_no' ).append(option);
				}
			});
		}
		});

		$(document.body).on('change', '#challan_no', function () {
			...
			$.ajax({
				type: "POST",
				url: "{{route('get_order_challan_detail')}}",
				data: {vendor_id: vendor_id,order_id:order_id,challan_no:challan_no},
