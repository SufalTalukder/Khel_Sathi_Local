...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ url('admin/addState') }}" class="needs-validation " id="reload" novalidate method="post">
                <div class="row">
                    <div class="col-md-4">
    ...
    <div class="pageheader" id="menu-margin">
        <h4 class="mb-0">State List
            <a title="State List" class="btn btn-sm btn-success float-end alphanumeric" onclick="ExportToExcel('xlsx')"><i
                        ...
                        <tbody>
                            @foreach ($states as $key => $item)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>
                                        <span id="stateup{{ $item->id }}"
                                            class="stateup{{ $item->id }}">{{ $item->name }}</span>
                                        <input style="display:none;" type="email"
                                            class="form-control state{{ $item->id }} alphanumeric"
                                            id="state{{ $item->id }}" value="{{ $item->name }}">
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <a class="btn btn-sm btn-dark stateup{{ $item->id }}" href="javascript:void(0)"
                                            onclick="showButton('{{ $item->id }}')">
                                            <i class="far fa-edit"></i>
                                        </a>
                                        <a style="display:none;" class="btn btn-sm btn-info state{{ $item->id }}"
                                            href="javascript:void(0)" onclick="updateState('{{ $item->id }}')">
                                            Update
                                        </a>
                                    </td>
                                </tr>
