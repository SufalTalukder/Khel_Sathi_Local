@extends('layouts/admin_layout')
@section('content')
    <link href="{{ asset('admin') }}/css/chat.css" rel='stylesheet' />
    <?= $cursor_prevent = '' ?>


    {{--
1 => Directorate( admin )
2 => RSO
3 => Associate
4 => Recruitment Cell
	 --}}
    {{-- {{dd(Auth::guard( 'admin' )->user()->admin_role)}} --}}
    @if (Auth::guard('admin')->user()->admin_role == 1 ||Auth::guard('admin')->user()->admin_role == 18 || Auth::guard('admin')->user()->admin_role == 5)
        <?php
        $cursor_prevent = '';
        $is_directorate = 1;
        $is_rso = 0;
        $is_associate = 1;
        $is_recruitment = 1;
        $is_college = 0;
        $is_hostel = 1;
        
        ?>
    @endif

    @if (Auth::guard('admin')->user()->admin_role == 2 || Auth::guard('admin')->user()->admin_role ==9)
        <?php
        $cursor_prevent = '';
        $is_directorate = 1;
        $is_rso = 1;
        $is_associate = 0;
        $is_recruitment = 0;
        $is_college = 0;
        $is_hostel = 0;
        ?>
    @endif

    @if (Auth::guard('admin')->user()->admin_role == 3)
        <?php
        $cursor_prevent = '';
        $is_directorate = 1;
        $is_rso = 0;
        $is_associate = 1;
        $is_recruitment = 1;
        $is_college = 0;
        $is_hostel = 0;
        ?>
    @endif

    @if (Auth::guard('admin')->user()->admin_role == 6)
        <?php
        $cursor_prevent = '';
        $is_directorate = 1;
        $is_rso = 0;
        $is_associate = 0;
        $is_recruitment = 0;
        $is_college = 1;
        $is_hostel = 0;
        ?>
    @endif



    {{-- @if (Auth::guard('admin')->user()->admin_role == 4 || Auth::guard('admin')->user()->admin_role == 9)
<?php
$cursor_prevent = '';
$is_directorate = 1;
$is_rso = 2;
$is_associate = 0;
$is_recruitment = 1;
$is_college = 0;
$is_hostel = 0;
?>
@endif --}}

    @if (Auth::guard('admin')->user()->admin_role == 11 || Auth::guard('admin')->user()->admin_role == 8)
        <?php
        $cursor_prevent = '';
        $is_directorate = 3;
        $is_rso = 0;
        $is_associate = 0;
        $is_recruitment = 0;
        $is_college = 0;
        $is_hostel = 0;
        ?>
    @endif

    @if (Auth::guard('admin')->user()->admin_role == 17)
        <?php
        $cursor_prevent = '';
        $is_directorate = 1;
        $is_rso = 0;
        $is_associate = 1;
        $is_recruitment = 1;
        $is_college = 0;

        
        ?>
    @endif

    <div class="row">
        <div class="col-md-12">
            <div class="pageheader" id="menu-margin">
                <div class="row">
                    <h4 class="col-md-10 mb-0">Dashboard</h4>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        @if (isset($is_directorate) && $is_directorate == 3)
                            <div class="col-md-3 {{ $cursor_prevent }}">
                                <a href="{{ route('direct_rect') }}" class="intentbtn bluecolor">
                                    <span class="intenticon">
                                        <svg fill="#000000" viewBox="0 0 512 512" enable-background="new 0 0 512 512"
                                            id="Management" version="1.1" xml:space="preserve"
                                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">

                                            <g>

                                                <path
                                                    d="M182.922,264.709c-39.745,0-72.08,32.336-72.08,72.08c0,39.745,32.335,72.079,72.08,72.079   c39.744,0,72.08-32.334,72.08-72.079c0-14.443-4.259-28.381-12.314-40.306c-2.095-3.099-6.305-3.915-9.406-1.821   c-3.1,2.096-3.916,6.306-1.821,9.406c6.538,9.677,9.993,20.99,9.993,32.721c0,32.274-26.257,58.531-58.531,58.531   c-32.275,0-58.531-26.257-58.531-58.531s26.256-58.531,58.531-58.531c3.741,0,6.774-3.033,6.774-6.773   C189.696,267.742,186.663,264.709,182.922,264.709z" />

                                                <path
                                                    d="M308.15,401.063l-17.715-17.717c-1.592-1.592-2.026-4.051-1.151-6.156c0.018-0.039,0.101-0.234,0.116-0.273   c0.914-2.206,2.956-3.632,5.203-3.632h25.054c3.74,0,6.774-3.033,6.774-6.774v-67.184c0-3.741-3.034-6.774-6.774-6.774h-25.054   c-2.247,0-4.289-1.426-5.273-3.798c-0.921-2.213-0.486-4.672,1.105-6.266l17.715-17.715c2.646-2.645,2.646-6.934,0-9.58   l-47.505-47.505c-2.646-2.645-6.935-2.645-9.581,0c-2.646,2.646-2.646,6.935,0,9.581l42.717,42.715l-12.925,12.924   c-5.487,5.486-7.071,13.748-3.973,21.195c3.017,7.287,9.972,11.997,17.72,11.997h18.279v53.635h-18.279   c-7.748,0-14.703,4.71-17.671,11.88c-0.015,0.035-0.097,0.228-0.111,0.264c-3.038,7.299-1.454,15.563,4.035,21.047l12.925,12.926   l-37.926,37.926l-12.925-12.926c-5.488-5.488-13.751-7.071-21.194-3.973c-7.288,3.018-11.998,9.975-11.998,17.721v18.279h-53.634   V444.6c0-7.747-4.709-14.703-12.144-17.781c-7.299-3.036-15.56-1.453-21.048,4.033l-12.925,12.926l-37.925-37.926l12.925-12.926   c5.486-5.488,7.07-13.75,3.973-21.193c-3.019-7.287-9.975-11.997-17.72-11.997H52.962v-53.635h18.279   c7.746,0,14.702-4.71,17.781-12.144c3.035-7.299,1.452-15.561-4.034-21.049l-12.925-12.924l37.925-37.925l12.925,12.924   c5.486,5.488,13.747,7.072,21.194,3.974c7.288-3.019,11.997-9.975,11.997-17.72v-18.279h53.634v18.279   c0,7.747,4.71,14.703,11.998,17.72c2.442,1.012,4.884,2.138,7.255,3.346c3.33,1.697,7.411,0.373,9.111-2.961   c1.698-3.333,0.373-7.412-2.961-9.111c-2.686-1.369-5.452-2.644-8.222-3.792c-2.207-0.914-3.633-2.956-3.633-5.203v-25.054   c0-3.742-3.033-6.774-6.774-6.774H149.33c-3.741,0-6.774,3.032-6.774,6.774v25.054c0,2.247-1.426,4.289-3.799,5.272   c-2.211,0.922-4.671,0.488-6.263-1.105l-17.715-17.715c-2.646-2.645-6.936-2.645-9.581,0l-47.506,47.505   c-2.646,2.646-2.646,6.936,0,9.582l17.715,17.713c1.593,1.594,2.026,4.053,1.128,6.213c-0.009,0.021-0.082,0.197-0.092,0.218   c-0.914,2.207-2.956,3.633-5.202,3.633H46.188c-3.741,0-6.774,3.033-6.774,6.774v67.184c0,3.741,3.033,6.774,6.774,6.774h25.054   c2.246,0,4.288,1.426,5.272,3.798c0.92,2.213,0.486,4.672-1.106,6.264l-17.715,17.715c-2.646,2.646-2.646,6.936,0,9.582   l47.506,47.505c2.646,2.645,6.935,2.645,9.581,0l17.715-17.716c1.592-1.594,4.051-2.023,6.211-1.127   c0.021,0.009,0.197,0.082,0.218,0.092c2.207,0.914,3.633,2.956,3.633,5.203v25.053c0,3.742,3.033,6.774,6.774,6.774h67.183   c3.741,0,6.774-3.032,6.774-6.774V444.6c0-2.248,1.426-4.29,3.557-5.174c0.029-0.012,0.215-0.088,0.243-0.1   c2.212-0.921,4.672-0.485,6.263,1.105l17.715,17.715c1.271,1.271,2.994,1.984,4.791,1.984c1.796,0,3.519-0.713,4.79-1.983   l47.505-47.505C310.796,407.996,310.796,403.707,308.15,401.063z" />

                                                <path
                                                    d="M377.926,312.506c0,24.05,19.566,43.617,43.618,43.617s43.618-19.567,43.618-43.617c0-24.051-19.566-43.619-43.618-43.619   S377.926,288.455,377.926,312.506z M451.613,312.506c0,16.58-13.489,30.069-30.069,30.069s-30.07-13.489-30.07-30.069   c0-16.582,13.49-30.07,30.07-30.07S451.613,295.924,451.613,312.506z" />

                                                <path
                                                    d="M465.813,443.785c3.74,0,6.774-3.032,6.774-6.773v-16.967c0-28.145-22.897-51.042-51.043-51.042   s-51.043,22.897-51.043,51.042v43.62h-82.472c-3.74,0-6.774,3.032-6.774,6.774c0,3.741,3.034,6.773,6.774,6.773h88.918   c0.111,0.006,0.216,0.033,0.328,0.033h88.537c3.74,0,6.774-3.033,6.774-6.773c0-3.742-3.034-6.775-6.774-6.775H384.05v-43.652   c0-20.674,16.82-37.494,37.494-37.494s37.494,16.82,37.494,37.494v16.967C459.038,440.753,462.072,443.785,465.813,443.785z" />

                                                <path
                                                    d="M93.609,185.172c3.741,0,6.774-3.033,6.774-6.774V64.49h44.934c3.741,0,6.774-3.032,6.774-6.774   c0-3.741-3.033-6.774-6.774-6.774H93.609c-3.741,0-6.774,3.033-6.774,6.774v120.682C86.835,182.14,89.868,185.172,93.609,185.172z" />

                                                <path
                                                    d="M375.443,64.49h45.933v163.937c0,3.742,3.033,6.774,6.774,6.774c3.74,0,6.773-3.033,6.773-6.774V57.716   c0-3.741-3.033-6.774-6.773-6.774h-52.707c-3.74,0-6.773,3.033-6.773,6.774C368.67,61.458,371.703,64.49,375.443,64.49z" />

                                                <path
                                                    d="M185.378,93.665h151.005c3.74,0,6.773-3.033,6.773-6.774V41.528c0-3.741-3.033-6.774-6.773-6.774H185.378   c-3.741,0-6.774,3.033-6.774,6.774v45.363C178.604,90.632,181.637,93.665,185.378,93.665z M192.152,48.302h137.456v31.814H192.152   V48.302z" />

                                                <path
                                                    d="M182.922,292.964c-3.742,0-6.774,3.033-6.774,6.774v37.051c0,3.741,3.032,6.773,6.774,6.773h24.714   c3.741,0,6.774-3.032,6.774-6.773c0-3.742-3.033-6.775-6.774-6.775h-17.939v-30.275   C189.696,295.997,186.663,292.964,182.922,292.964z" />

                                                <path
                                                    d="M168.652,146.977h152.94c3.74,0,6.773-3.032,6.773-6.774c0-3.741-3.033-6.774-6.773-6.774h-152.94   c-3.742,0-6.774,3.033-6.774,6.774C161.878,143.944,164.91,146.977,168.652,146.977z" />

                                                <path
                                                    d="M321.593,188.101h-31.064c-3.74,0-6.774,3.033-6.774,6.774c0,3.741,3.034,6.774,6.774,6.774h31.064   c3.74,0,6.773-3.033,6.773-6.774C328.366,191.134,325.333,188.101,321.593,188.101z" />

                                                <path
                                                    d="M362.332,154.588c1.323,1.322,3.057,1.983,4.789,1.983c1.734,0,3.469-0.662,4.79-1.984l19.19-19.191   c2.646-2.645,2.646-6.934,0-9.58c-2.646-2.644-6.937-2.645-9.579,0.001l-14.401,14.401l-5.706-5.705   c-2.646-2.645-6.937-2.646-9.579,0.001c-2.646,2.645-2.646,6.935,0,9.58L362.332,154.588z" />

                                                <path
                                                    d="M362.332,209.261c1.323,1.322,3.057,1.984,4.789,1.984c1.734,0,3.469-0.662,4.79-1.985l19.19-19.191   c2.646-2.645,2.646-6.934,0-9.58c-2.646-2.644-6.937-2.645-9.579,0.001l-14.401,14.401l-5.706-5.705   c-2.646-2.645-6.937-2.646-9.579,0c-2.646,2.645-2.646,6.935,0,9.58L362.332,209.261z" />

                                            </g>

                                        </svg>
                                    </span>
                                    <span class="intentdata"><b class="intentno">
                                            @if (isset($direct))
                                                {{ $direct[0]->total }}
                                            @endif
                                        </b>Direct Recruitment</span>
                                </a>
                            </div>
                        @endif
                        @if (isset($is_recruitment) && ($is_recruitment == 1 && $is_directorate == 1) && Auth::guard('admin')->user()->admin_role != 17)
                            <div class="col-md-3 {{ $cursor_prevent }}">
                                <a href="{{ route('direct_rect') }}" class="intentbtn bluecolor">
                                    <span class="intenticon">
                                        <svg fill="#000000" viewBox="0 0 512 512" enable-background="new 0 0 512 512"
                                            id="Management" version="1.1" xml:space="preserve"
                                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">

                                            <g>

                                                <path
                                                    d="M182.922,264.709c-39.745,0-72.08,32.336-72.08,72.08c0,39.745,32.335,72.079,72.08,72.079   c39.744,0,72.08-32.334,72.08-72.079c0-14.443-4.259-28.381-12.314-40.306c-2.095-3.099-6.305-3.915-9.406-1.821   c-3.1,2.096-3.916,6.306-1.821,9.406c6.538,9.677,9.993,20.99,9.993,32.721c0,32.274-26.257,58.531-58.531,58.531   c-32.275,0-58.531-26.257-58.531-58.531s26.256-58.531,58.531-58.531c3.741,0,6.774-3.033,6.774-6.773   C189.696,267.742,186.663,264.709,182.922,264.709z" />

                                                <path
                                                    d="M308.15,401.063l-17.715-17.717c-1.592-1.592-2.026-4.051-1.151-6.156c0.018-0.039,0.101-0.234,0.116-0.273   c0.914-2.206,2.956-3.632,5.203-3.632h25.054c3.74,0,6.774-3.033,6.774-6.774v-67.184c0-3.741-3.034-6.774-6.774-6.774h-25.054   c-2.247,0-4.289-1.426-5.273-3.798c-0.921-2.213-0.486-4.672,1.105-6.266l17.715-17.715c2.646-2.645,2.646-6.934,0-9.58   l-47.505-47.505c-2.646-2.645-6.935-2.645-9.581,0c-2.646,2.646-2.646,6.935,0,9.581l42.717,42.715l-12.925,12.924   c-5.487,5.486-7.071,13.748-3.973,21.195c3.017,7.287,9.972,11.997,17.72,11.997h18.279v53.635h-18.279   c-7.748,0-14.703,4.71-17.671,11.88c-0.015,0.035-0.097,0.228-0.111,0.264c-3.038,7.299-1.454,15.563,4.035,21.047l12.925,12.926   l-37.926,37.926l-12.925-12.926c-5.488-5.488-13.751-7.071-21.194-3.973c-7.288,3.018-11.998,9.975-11.998,17.721v18.279h-53.634   V444.6c0-7.747-4.709-14.703-12.144-17.781c-7.299-3.036-15.56-1.453-21.048,4.033l-12.925,12.926l-37.925-37.926l12.925-12.926   c5.486-5.488,7.07-13.75,3.973-21.193c-3.019-7.287-9.975-11.997-17.72-11.997H52.962v-53.635h18.279   c7.746,0,14.702-4.71,17.781-12.144c3.035-7.299,1.452-15.561-4.034-21.049l-12.925-12.924l37.925-37.925l12.925,12.924   c5.486,5.488,13.747,7.072,21.194,3.974c7.288-3.019,11.997-9.975,11.997-17.72v-18.279h53.634v18.279   c0,7.747,4.71,14.703,11.998,17.72c2.442,1.012,4.884,2.138,7.255,3.346c3.33,1.697,7.411,0.373,9.111-2.961   c1.698-3.333,0.373-7.412-2.961-9.111c-2.686-1.369-5.452-2.644-8.222-3.792c-2.207-0.914-3.633-2.956-3.633-5.203v-25.054   c0-3.742-3.033-6.774-6.774-6.774H149.33c-3.741,0-6.774,3.032-6.774,6.774v25.054c0,2.247-1.426,4.289-3.799,5.272   c-2.211,0.922-4.671,0.488-6.263-1.105l-17.715-17.715c-2.646-2.645-6.936-2.645-9.581,0l-47.506,47.505   c-2.646,2.646-2.646,6.936,0,9.582l17.715,17.713c1.593,1.594,2.026,4.053,1.128,6.213c-0.009,0.021-0.082,0.197-0.092,0.218   c-0.914,2.207-2.956,3.633-5.202,3.633H46.188c-3.741,0-6.774,3.033-6.774,6.774v67.184c0,3.741,3.033,6.774,6.774,6.774h25.054   c2.246,0,4.288,1.426,5.272,3.798c0.92,2.213,0.486,4.672-1.106,6.264l-17.715,17.715c-2.646,2.646-2.646,6.936,0,9.582   l47.506,47.505c2.646,2.645,6.935,2.645,9.581,0l17.715-17.716c1.592-1.594,4.051-2.023,6.211-1.127   c0.021,0.009,0.197,0.082,0.218,0.092c2.207,0.914,3.633,2.956,3.633,5.203v25.053c0,3.742,3.033,6.774,6.774,6.774h67.183   c3.741,0,6.774-3.032,6.774-6.774V444.6c0-2.248,1.426-4.29,3.557-5.174c0.029-0.012,0.215-0.088,0.243-0.1   c2.212-0.921,4.672-0.485,6.263,1.105l17.715,17.715c1.271,1.271,2.994,1.984,4.791,1.984c1.796,0,3.519-0.713,4.79-1.983   l47.505-47.505C310.796,407.996,310.796,403.707,308.15,401.063z" />

                                                <path
                                                    d="M377.926,312.506c0,24.05,19.566,43.617,43.618,43.617s43.618-19.567,43.618-43.617c0-24.051-19.566-43.619-43.618-43.619   S377.926,288.455,377.926,312.506z M451.613,312.506c0,16.58-13.489,30.069-30.069,30.069s-30.07-13.489-30.07-30.069   c0-16.582,13.49-30.07,30.07-30.07S451.613,295.924,451.613,312.506z" />

                                                <path
                                                    d="M465.813,443.785c3.74,0,6.774-3.032,6.774-6.773v-16.967c0-28.145-22.897-51.042-51.043-51.042   s-51.043,22.897-51.043,51.042v43.62h-82.472c-3.74,0-6.774,3.032-6.774,6.774c0,3.741,3.034,6.773,6.774,6.773h88.918   c0.111,0.006,0.216,0.033,0.328,0.033h88.537c3.74,0,6.774-3.033,6.774-6.773c0-3.742-3.034-6.775-6.774-6.775H384.05v-43.652   c0-20.674,16.82-37.494,37.494-37.494s37.494,16.82,37.494,37.494v16.967C459.038,440.753,462.072,443.785,465.813,443.785z" />

                                                <path
                                                    d="M93.609,185.172c3.741,0,6.774-3.033,6.774-6.774V64.49h44.934c3.741,0,6.774-3.032,6.774-6.774   c0-3.741-3.033-6.774-6.774-6.774H93.609c-3.741,0-6.774,3.033-6.774,6.774v120.682C86.835,182.14,89.868,185.172,93.609,185.172z" />

                                                <path
                                                    d="M375.443,64.49h45.933v163.937c0,3.742,3.033,6.774,6.774,6.774c3.74,0,6.773-3.033,6.773-6.774V57.716   c0-3.741-3.033-6.774-6.773-6.774h-52.707c-3.74,0-6.773,3.033-6.773,6.774C368.67,61.458,371.703,64.49,375.443,64.49z" />

                                                <path
                                                    d="M185.378,93.665h151.005c3.74,0,6.773-3.033,6.773-6.774V41.528c0-3.741-3.033-6.774-6.773-6.774H185.378   c-3.741,0-6.774,3.033-6.774,6.774v45.363C178.604,90.632,181.637,93.665,185.378,93.665z M192.152,48.302h137.456v31.814H192.152   V48.302z" />

                                                <path
                                                    d="M182.922,292.964c-3.742,0-6.774,3.033-6.774,6.774v37.051c0,3.741,3.032,6.773,6.774,6.773h24.714   c3.741,0,6.774-3.032,6.774-6.773c0-3.742-3.033-6.775-6.774-6.775h-17.939v-30.275   C189.696,295.997,186.663,292.964,182.922,292.964z" />

                                                <path
                                                    d="M168.652,146.977h152.94c3.74,0,6.773-3.032,6.773-6.774c0-3.741-3.033-6.774-6.773-6.774h-152.94   c-3.742,0-6.774,3.033-6.774,6.774C161.878,143.944,164.91,146.977,168.652,146.977z" />

                                                <path
                                                    d="M321.593,188.101h-31.064c-3.74,0-6.774,3.033-6.774,6.774c0,3.741,3.034,6.774,6.774,6.774h31.064   c3.74,0,6.773-3.033,6.773-6.774C328.366,191.134,325.333,188.101,321.593,188.101z" />

                                                <path
                                                    d="M362.332,154.588c1.323,1.322,3.057,1.983,4.789,1.983c1.734,0,3.469-0.662,4.79-1.984l19.19-19.191   c2.646-2.645,2.646-6.934,0-9.58c-2.646-2.644-6.937-2.645-9.579,0.001l-14.401,14.401l-5.706-5.705   c-2.646-2.645-6.937-2.646-9.579,0.001c-2.646,2.645-2.646,6.935,0,9.58L362.332,154.588z" />

                                                <path
                                                    d="M362.332,209.261c1.323,1.322,3.057,1.984,4.789,1.984c1.734,0,3.469-0.662,4.79-1.985l19.19-19.191   c2.646-2.645,2.646-6.934,0-9.58c-2.646-2.644-6.937-2.645-9.579,0.001l-14.401,14.401l-5.706-5.705   c-2.646-2.645-6.937-2.646-9.579,0c-2.646,2.645-2.646,6.935,0,9.58L362.332,209.261z" />

                                            </g>

                                        </svg>
                                    </span>
                                    <span class="intentdata"><b class="intentno">
                                            @if (isset($direct))
                                                {{ $direct[0]->total }}
                                            @endif
                                        </b>Direct Recruitment</span>
                                </a>
                            </div>
                        @endif
                        @if (isset($is_directorate) && $is_directorate == 1 && $is_rso == 0 && $is_college == 0)
                            <div class="col-md-3 {{ $cursor_prevent }}">
                                <a href="{{ route('financial_assis') }}" class="intentbtn pinkcolor">
                                    <span class="intenticon">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
                                            <g>
                                                <path
                                                    d="M98.925,91.96V94a6.92,6.92,0,0,0,13.84,0V91.96a6.92,6.92,0,1,0-13.84,0Zm10.34,0V94a3.42,3.42,0,0,1-6.84,0V91.96a3.42,3.42,0,1,1,6.84,0Z" />
                                                <path
                                                    d="M22.155,85.039a6.929,6.929,0,0,0-6.92,6.921V94a6.92,6.92,0,0,0,13.84,0V91.96A6.928,6.928,0,0,0,22.155,85.039ZM25.575,94a3.42,3.42,0,0,1-6.84,0V91.96a3.42,3.42,0,1,1,6.84,0Z" />
                                                <path
                                                    d="M64,81.213a8.443,8.443,0,0,0-8.433,8.433v2.632a8.433,8.433,0,0,0,16.866,0V89.646A8.443,8.443,0,0,0,64,81.213Zm4.933,11.065a4.933,4.933,0,1,1-9.866,0V89.646a4.933,4.933,0,0,1,9.866,0Z" />
                                                <path
                                                    d="M126.25,16.747A1.749,1.749,0,0,0,128,15V1.747A1.749,1.749,0,0,0,126.25,0H1.75A1.749,1.749,0,0,0,0,1.747V15a1.749,1.749,0,0,0,1.75,1.75h8.844v64.23a22.145,22.145,0,1,0,28.894,32.657,28.1,28.1,0,0,0,49.022,0,22.145,22.145,0,1,0,28.9-32.661V16.747ZM3.5,3.5h121v9.75H3.5ZM10.133,114.11a7.026,7.026,0,0,1,7.009-6.725H27.169a7.025,7.025,0,0,1,7.009,6.741,18.609,18.609,0,0,1-24.045-.016Zm26.976-3.125a10.533,10.533,0,0,0-9.94-7.1H17.142A10.532,10.532,0,0,0,7.2,110.977,18.629,18.629,0,1,1,37.778,89.718a28.018,28.018,0,0,0,0,20.3Q37.457,110.515,37.109,110.985ZM47.93,118.5v-2.429a9.6,9.6,0,0,1,9.589-9.59H70.48a9.6,9.6,0,0,1,9.59,9.59V118.5a24.551,24.551,0,0,1-32.14,0Zm35.579-3.634A13.1,13.1,0,0,0,70.48,102.984H57.519a13.1,13.1,0,0,0-13.028,11.885,24.625,24.625,0,1,1,39.018,0Zm10.313-.737a7.024,7.024,0,0,1,7.009-6.747h10.027a7.026,7.026,0,0,1,7.009,6.725,18.619,18.619,0,0,1-24.045.022ZM124.5,99.868a18.535,18.535,0,0,1-3.7,11.109,10.532,10.532,0,0,0-9.938-7.092H100.831a10.533,10.533,0,0,0-9.94,7.1q-.348-.469-.669-.965a28.021,28.021,0,0,0,0-20.3A18.638,18.638,0,0,1,124.5,99.868Zm-10.594-20.63a22.094,22.094,0,0,0-25.4,6.863,28.1,28.1,0,0,0-49.019,0,22.1,22.1,0,0,0-25.4-6.858V16.747h99.812Z" />
                                                <path
                                                    d="M20.344,33.748h87.312a1.749,1.749,0,0,0,1.75-1.75V24a1.749,1.749,0,0,0-1.75-1.75H20.344A1.749,1.749,0,0,0,18.594,24v8A1.749,1.749,0,0,0,20.344,33.748Zm1.75-8h83.812v4.5H22.094Z" />
                                                <path
                                                    d="M20.344,50.314h87.312a1.75,1.75,0,0,0,1.75-1.75v-8a1.749,1.749,0,0,0-1.75-1.75H20.344a1.749,1.749,0,0,0-1.75,1.75v8A1.75,1.75,0,0,0,20.344,50.314Zm1.75-8h83.812v4.5H22.094Z" />
                                                <path
                                                    d="M103.344,55.379H24.656a1.749,1.749,0,0,0-1.75,1.75v8a1.749,1.749,0,0,0,1.75,1.75h78.688a1.75,1.75,0,0,0,1.75-1.75v-8A1.75,1.75,0,0,0,103.344,55.379Zm-1.75,8H26.406v-4.5h75.188Z" />
                                            </g>
                                        </svg>
                                    </span>
                                    <span class="intentdata"><b class="intentno">
                                            @if (isset($financial))
                                                {{ $financial[0]->total }}
                                            @endif
                                        </b>Financial Assistance</span>
                                </a>
                            </div>
                            <div class="col-md-3 {{ $cursor_prevent }}">
                                <a href="{{ route('monthly_pension') }}" class="intentbtn yellowcolor">
                                    <span class="intenticon">
                                        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512"
                                            xml:space="preserve" stroke="#ffffff">
                                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round">
                                            </g>
                                            <g id="SVGRepo_iconCarrier">
                                                <g>
                                                    <g>
                                                        <g>
                                                            <path
                                                                d="M458.666,42.67h-53.33V32c0-17.645-14.356-32-32.002-32c-17.644,0-31.999,14.355-31.999,32v10.67H170.664V32 c0-17.645-14.354-32-31.999-32s-32,14.355-32,32v10.67H53.334c-5.892,0-10.667,4.776-10.667,10.667v447.995 c0,5.89,4.776,10.667,10.667,10.667h405.332c5.891,0,10.667-4.778,10.667-10.667V53.337 C469.333,47.446,464.557,42.67,458.666,42.67z M362.67,53.337V32c0-5.882,4.784-10.665,10.666-10.665 c5.881,0,10.665,4.783,10.665,10.665v21.337V74.67c0,0.735-0.075,1.452-0.218,2.146c-0.996,4.855-5.303,8.517-10.45,8.517 c-5.881,0-10.664-4.783-10.664-10.663V53.337z M128,53.337V32c0-5.882,4.783-10.665,10.667-10.665 c5.88,0,10.662,4.783,10.662,10.665v21.337V74.67c0,1.469-0.299,2.871-0.838,4.146c-1.621,3.825-5.415,6.517-9.826,6.517 C132.783,85.333,128,80.55,128,74.67V53.337z M64.001,64.005h42.663V74.67c0,2.756,0.35,5.434,1.009,7.988 c3.557,13.791,16.103,24.01,30.991,24.01h0.002c17.643,0,31.997-14.355,31.997-31.998V64.005h170.67V74.67 c0,17.643,14.355,31.998,32.001,31.998c17.645,0,32-14.355,32-31.998V64.005h42.662v63.994H64.001V64.005z M447.999,490.665 H64.001v0v-21.328h68.657c5.891,0,10.667-4.778,10.667-10.667c0-5.892-4.777-10.667-10.667-10.667H64.001V149.334h383.997 V490.665z">
                                                            </path>
                                                            <path
                                                                d="M141.432,249.712c1.736,0,4.342-0.868,6.37-2.896l10.134-12.742v160.764c0,6.661,7.528,10.134,15.347,10.134 c7.531,0,15.349-3.473,15.349-10.134v-191.41c-0.001-6.371-7.24-10.134-13.612-10.134c-3.474,0-5.792,1.159-7.818,3.185 l-30.115,28.907c-3.765,2.608-6.08,7.53-6.08,11.874C131.007,243.34,135.349,249.712,141.432,249.712z">
                                                            </path>
                                                            <path
                                                                d="M316.134,406.711c36.486,0,64.866-16.506,64.866-59.652v-3.475c-0.001-29.827-13.321-46.913-33.303-54.154 c16.216-6.082,27.221-20.556,27.221-45.174c0-37.065-24.904-50.962-58.784-50.962c-33.881,0-58.784,13.896-58.784,50.962 c0,24.618,11.005,39.092,26.931,45.174c-19.98,7.24-33.301,24.327-33.301,54.154v3.475 C250.98,390.206,279.647,406.711,316.134,406.711z M316.134,218.774c18.245,0,28.959,8.398,28.959,28.958 c0,20.85-10.714,29.248-28.959,29.248c-18.242,0-28.958-8.398-28.958-29.248C287.175,227.173,297.892,218.774,316.134,218.774z M281.674,338.66c0-24.904,13.032-36.197,34.46-36.197c21.428,0,34.17,11.293,34.17,36.197v5.213 c0,25.191-12.451,37.353-34.17,37.353c-21.139,0-34.46-11.58-34.46-37.353V338.66z">
                                                            </path>
                                                            <path
                                                                d="M163.556,448.006h-0.254c-5.892,0-10.667,4.776-10.667,10.667c0,5.889,4.776,10.667,10.667,10.667h0.254 c5.892,0,10.667-4.778,10.667-10.667C174.224,452.781,169.448,448.006,163.556,448.006z">
                                                            </path>
                                                        </g>
                                                    </g>
                                                </g>
                                            </g>
                                        </svg>
                                    </span>
                                    <span class="intentdata"><b class="intentno">
                                            @if (isset($monthly))
                                                {{ $monthly[0]->total }}
                                            @endif
                                        </b> Monthly Pension</span>
                                </a>
                            </div>
                            <div class="col-md-3 {{ $cursor_prevent }}">
                                <a href="{{ route('laxman') }}" class="intentbtn greencolor">
                                    <span class="intenticon">
                                        <svg fill="#ffffff" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
                                            stroke="#ffffff">
                                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round">
                                            </g>
                                            <g id="SVGRepo_iconCarrier">
                                                <g data-name="Layer 2">
                                                    <g data-name="award">
                                                        <rect width="24" height="24" opacity="0"></rect>
                                                        <path
                                                            d="M19 20.75l-2.31-9A5.94 5.94 0 0 0 18 8 6 6 0 0 0 6 8a5.94 5.94 0 0 0 1.34 3.77L5 20.75a1 1 0 0 0 1.48 1.11l5.33-3.13 5.68 3.14A.91.91 0 0 0 18 22a1 1 0 0 0 1-1.25zM12 4a4 4 0 1 1-4 4 4 4 0 0 1 4-4z">
                                                        </path>
                                                    </g>
                                                </g>
                                            </g>
                                        </svg>
                                    </span>
                                    <span class="intentdata"> <b class="intentno">
                                            @if (isset($laxman))
                                                {{ $laxman[0]->total }}
                                            @endif
                                        </b> Nomination for Laxman Award</span>
                                </a>
                            </div>
                            <div class="col-md-3 {{ $cursor_prevent }}">
                                <a href="{{ route('laxmibai') }}" class="intentbtn box5color">
                                    <span class="intenticon">
                                        <svg fill="#000000" viewBox="0 0 32 32" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M31.908 26.249l-5.852-10.822c0.597-1.355 0.931-2.852 0.931-4.428 0-6.072-4.922-10.994-10.994-10.994-6.073 0-10.995 4.923-10.995 10.994 0 1.614 0.351 3.145 0.974 4.524l-5.878 10.721c-0.19 0.345-0.158 0.77 0.079 1.084 0.237 0.314 0.638 0.461 1.022 0.371l5.019-1.151 1.718 4.785c0.134 0.372 0.474 0.63 0.867 0.659 0.025 0.002 0.050 0.003 0.074 0.003 0.366 0 0.706-0.201 0.881-0.527l5.116-9.53c0.369 0.038 0.744 0.057 1.123 0.057 0.347 0 0.69-0.018 1.029-0.050l5.227 9.532c0.177 0.323 0.514 0.52 0.877 0.52 0.026 0 0.052-0.001 0.078-0.003 0.392-0.032 0.73-0.289 0.863-0.659l1.718-4.785 5.020 1.151c0.385 0.093 0.782-0.057 1.020-0.369s0.27-0.735 0.084-1.081zM9.056 28.542l-1.258-3.505c-0.172-0.477-0.671-0.754-1.165-0.637l-3.712 0.852 4.231-7.718c1.393 1.883 3.373 3.303 5.67 3.994zM7.007 10.999c0-4.955 4.032-8.986 8.986-8.986s8.985 4.031 8.985 8.986-4.031 8.986-8.986 8.986c-4.955 0-8.986-4.032-8.986-8.986zM25.367 24.4c-0.496-0.117-0.993 0.16-1.165 0.636l-1.267 3.53-3.849-7.017c2.357-0.691 4.386-2.148 5.797-4.085l4.214 7.791z">
                                            </path>
                                        </svg>
                                    </span>
                                    <span class="intentdata"> <b class="intentno">
                                            @if (isset($ranilaxmibai))
                                                {{ $ranilaxmibai[0]->total }}
                                            @endif
                                        </b> Nomination for Rani Laxmibai Award</span>
                                </a>
                            </div>
                            @endif @if ((isset($is_directorate) && ($is_directorate == 1 && $is_rso == 1)) || (isset($is_associate) && $is_associate == 1))
                                <div class="col-md-3 {{ $cursor_prevent }}">
                                    <a href="{{ route('award') }}" class="intentbtn box6color">
                                        <span class="intenticon">
                                            <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                                                xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 493.133 493.133"
                                                xml:space="preserve" stroke="#ffffff">
                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                                    stroke-linejoin="round"></g>
                                                <g id="SVGRepo_iconCarrier">
                                                    <g>
                                                        <path
                                                            d="M416.588,104.553c-2.867-2.858-7.512-2.858-10.379,0.008l-1.065,1.066l-4.126-4.125c-1.184-1.187-2.785-1.858-4.468-1.858 c-1.339,0-2.565,0.526-3.631,1.297c1.208,6.148,0.647,12.651-1.914,18.819c-0.188,0.451-14.62,31.573-14.62,31.573 c0.219,0.02,0.418,0.128,0.641,0.128c1.953,0,3.81-0.77,5.189-2.147l34.389-34.366c1.377-1.377,2.147-3.25,2.147-5.198 C418.75,107.805,417.965,105.94,416.588,104.553z">
                                                        </path>
                                                        <path
                                                            d="M358.735,53.324c-0.488-0.646-0.426-1.555,0.147-2.127l4.513-4.514c6.711-6.704,8.456-16.939,4.357-25.477L361.375,7.88 c-1.877-3.913-6.578-6.606-9.822-7.457c-1.199-0.314-6.41-1.826-11.914,3.74c-2.297,2.324-9.24,9.244-9.24,9.244 c-0.556,0.557-1.477,0.489-1.947-0.143l-0.054-0.074c-1.085-1.463-2.47-2.711-4.12-3.481c-4.322-2.014-9.177-1.024-12.315,2.119 l-32.935,32.948c-2.209,2.21-3.363,5.269-3.141,8.384c0.209,3.116,1.779,5.992,4.263,7.873l0.015,0.012 c0.785,0.6,0.86,1.753,0.162,2.451l-8.939,8.941c-3.107,3.084-4.532,7.505-3.796,11.829c0.736,4.309,3.523,8.009,7.464,9.907 l13.343,6.383c2.996,1.442,6.343,2.203,9.659,2.203c5.975,0,11.581-2.322,15.825-6.544l4.522-4.523 c0.573-0.574,1.482-0.637,2.129-0.148l7.572,5.721c0.284,0.215,0.631,0.268,0.923,0.463l2.073-4.428 c6.346-13.552,20.787-20.659,34.834-18.425c0.897,0.143,1.738-0.518,1.837-1.421c0.478-4.354-0.487-8.825-3.308-12.55 L358.735,53.324z M308.173,84.522l-2.987,2.989c-2.979,2.979-7.656,3.804-11.469,1.954l-12.713-6.117 c-1.007-0.484-1.236-1.816-0.448-2.609l10.013-10.073c0.572-0.576,1.482-0.641,2.131-0.151l15.304,11.563 C308.79,82.672,308.87,83.825,308.173,84.522z M354.696,37.976l-2.984,2.987c-0.697,0.698-1.85,0.618-2.445-0.169l-11.564-15.3 c-0.488-0.646-0.426-1.554,0.146-2.127c0,0,9.518-9.527,10.437-10.469c0.763-0.781,1.372-0.359,1.685,0.25 c0.255,0.497,6.697,13.383,6.697,13.383C358.508,30.368,357.723,34.965,354.696,37.976z">
                                                        </path>
                                                        <path
                                                            d="M255.352,183.663l-27.5-0.145l-27.605-0.146l9.441,26.194l12.924,35.855c1.691-0.387,3.434-0.634,5.24-0.634 c1.807,0,3.549,0.247,5.24,0.634l12.924-35.855L255.352,183.663z">
                                                        </path>
                                                        <path
                                                            d="M369.512,88.648c-10.299-4.829-22.473-0.384-27.262,9.867l-24.812,52.971c-3.38,7.232-9.241,13.046-16.514,16.386 l-5.134,2.351l-26.98,12.354l-24.77,68.733c4.528,4.3,7.389,10.336,7.389,17.057c0,12.999-10.57,23.578-23.577,23.578 c-13.006,0-23.579-10.579-23.579-23.578c0-6.722,2.862-12.758,7.392-17.058l-24.277-67.364l-0.493-1.368l-32.114-14.704 c-7.271-3.34-13.135-9.154-16.53-16.402l-24.796-52.954c-4.789-10.251-16.994-14.696-27.262-9.867 c-10.251,4.798-14.672,17.003-9.867,27.263l24.779,52.938c7.496,16.026,20.487,28.92,36.602,36.296l39.867,18.12v248.871 c0,11.597,9.401,20.998,20.999,20.998c11.597,0,20.998-9.401,20.998-20.998V346.637h16.596v125.491 c0,11.596,9.402,20.999,20.998,20.999c11.582,0,20.983-9.395,20.983-20.983V223.26L318,205.145 c16.113-7.376,29.103-20.271,36.584-36.28l24.795-52.954C384.184,105.651,379.764,93.446,369.512,88.648z">
                                                        </path>
                                                        <path
                                                            d="M227.868,170.305c25.229,0,45.684-20.461,45.684-45.697c0-25.245-20.455-45.706-45.684-45.706 c-25.26,0-45.714,20.461-45.714,45.706C182.154,149.844,202.608,170.305,227.868,170.305z">
                                                        </path>
                                                        <path
                                                            d="M212.474,268.366c0,8.49,6.89,15.377,15.378,15.377c8.489,0,15.377-6.887,15.377-15.377 c0-8.489-6.888-15.377-15.377-15.377C219.364,252.988,212.474,259.877,212.474,268.366z">
                                                        </path>
                                                    </g>
                                                </g>
                                            </svg>
                                        </span>
                                        <span class="intentdata"><b class="intentno">
                                                @if (isset($position))
                                                    {{ $position[0]->total }}
                                                @endif
                                            </b>Nomination for Prize Money</span>
                                    </a>
                                </div>
                                <div class="col-md-3 {{ $cursor_prevent }}">
                                    @if(Auth::guard('admin')->user()->admin_role == 17)
                                        <a href="{{ route('eklavya_krida_co') }}/6" class="intentbtn pinkcolor">
                                    @else
                                        <a href="{{ route('eklavya_krida') }}" class="intentbtn pinkcolor">
                                    @endif
                                        <span class="intenticon">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
                                                <g>
                                                    <path
                                                        d="M98.925,91.96V94a6.92,6.92,0,0,0,13.84,0V91.96a6.92,6.92,0,1,0-13.84,0Zm10.34,0V94a3.42,3.42,0,0,1-6.84,0V91.96a3.42,3.42,0,1,1,6.84,0Z" />
                                                    <path
                                                        d="M22.155,85.039a6.929,6.929,0,0,0-6.92,6.921V94a6.92,6.92,0,0,0,13.84,0V91.96A6.928,6.928,0,0,0,22.155,85.039ZM25.575,94a3.42,3.42,0,0,1-6.84,0V91.96a3.42,3.42,0,1,1,6.84,0Z" />
                                                    <path
                                                        d="M64,81.213a8.443,8.443,0,0,0-8.433,8.433v2.632a8.433,8.433,0,0,0,16.866,0V89.646A8.443,8.443,0,0,0,64,81.213Zm4.933,11.065a4.933,4.933,0,1,1-9.866,0V89.646a4.933,4.933,0,0,1,9.866,0Z" />
                                                    <path
                                                        d="M126.25,16.747A1.749,1.749,0,0,0,128,15V1.747A1.749,1.749,0,0,0,126.25,0H1.75A1.749,1.749,0,0,0,0,1.747V15a1.749,1.749,0,0,0,1.75,1.75h8.844v64.23a22.145,22.145,0,1,0,28.894,32.657,28.1,28.1,0,0,0,49.022,0,22.145,22.145,0,1,0,28.9-32.661V16.747ZM3.5,3.5h121v9.75H3.5ZM10.133,114.11a7.026,7.026,0,0,1,7.009-6.725H27.169a7.025,7.025,0,0,1,7.009,6.741,18.609,18.609,0,0,1-24.045-.016Zm26.976-3.125a10.533,10.533,0,0,0-9.94-7.1H17.142A10.532,10.532,0,0,0,7.2,110.977,18.629,18.629,0,1,1,37.778,89.718a28.018,28.018,0,0,0,0,20.3Q37.457,110.515,37.109,110.985ZM47.93,118.5v-2.429a9.6,9.6,0,0,1,9.589-9.59H70.48a9.6,9.6,0,0,1,9.59,9.59V118.5a24.551,24.551,0,0,1-32.14,0Zm35.579-3.634A13.1,13.1,0,0,0,70.48,102.984H57.519a13.1,13.1,0,0,0-13.028,11.885,24.625,24.625,0,1,1,39.018,0Zm10.313-.737a7.024,7.024,0,0,1,7.009-6.747h10.027a7.026,7.026,0,0,1,7.009,6.725,18.619,18.619,0,0,1-24.045.022ZM124.5,99.868a18.535,18.535,0,0,1-3.7,11.109,10.532,10.532,0,0,0-9.938-7.092H100.831a10.533,10.533,0,0,0-9.94,7.1q-.348-.469-.669-.965a28.021,28.021,0,0,0,0-20.3A18.638,18.638,0,0,1,124.5,99.868Zm-10.594-20.63a22.094,22.094,0,0,0-25.4,6.863,28.1,28.1,0,0,0-49.019,0,22.1,22.1,0,0,0-25.4-6.858V16.747h99.812Z" />
                                                    <path
                                                        d="M20.344,33.748h87.312a1.749,1.749,0,0,0,1.75-1.75V24a1.749,1.749,0,0,0-1.75-1.75H20.344A1.749,1.749,0,0,0,18.594,24v8A1.749,1.749,0,0,0,20.344,33.748Zm1.75-8h83.812v4.5H22.094Z" />
                                                    <path
                                                        d="M20.344,50.314h87.312a1.75,1.75,0,0,0,1.75-1.75v-8a1.749,1.749,0,0,0-1.75-1.75H20.344a1.749,1.749,0,0,0-1.75,1.75v8A1.75,1.75,0,0,0,20.344,50.314Zm1.75-8h83.812v4.5H22.094Z" />
                                                    <path
                                                        d="M103.344,55.379H24.656a1.749,1.749,0,0,0-1.75,1.75v8a1.749,1.749,0,0,0,1.75,1.75h78.688a1.75,1.75,0,0,0,1.75-1.75v-8A1.75,1.75,0,0,0,103.344,55.379Zm-1.75,8H26.406v-4.5h75.188Z" />
                                                </g>
                                            </svg>
                                        </span>
                                        <span class="intentdata"><b class="intentno">
                                                @if (isset($eklavya))
                                                    {{ $eklavya[0]->total }}
                                                @endif
                                            </b>Eklavya Krida Kosh </span>
                                    </a>
                                </div>
                                @endif @if (isset($onlineAdmission) &&
                                        isset($is_directorate) &&
                                        $is_directorate == 1 &&
                                        ($is_college == 1 || Auth::guard('admin')->user()->admin_role == 1 || Auth::guard('admin')->user()->admin_role == 18))
                                    <div class="col-md-3 {{ $cursor_prevent }}">
                                        <a href="{{ route('collegeadmindashboard') }}" class="intentbtn bluecolor">
                                            <span class="intenticon">
                                                <svg fill="#000000" viewBox="0 0 1024 1024"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M263.508 346.359c0-11.782 9.551-21.333 21.333-21.333h303.012c11.78 0 21.333 9.551 21.333 21.333s-9.553 21.333-21.333 21.333H284.841c-11.782 0-21.333-9.551-21.333-21.333zm21.333 92.937c-11.782 0-21.333 9.553-21.333 21.333 0 11.785 9.551 21.333 21.333 21.333h303.012c11.78 0 21.333-9.549 21.333-21.333 0-11.78-9.553-21.333-21.333-21.333H284.841zm-21.333 135.599c0-11.78 9.551-21.333 21.333-21.333h303.012c11.78 0 21.333 9.553 21.333 21.333 0 11.785-9.553 21.333-21.333 21.333H284.841c-11.782 0-21.333-9.549-21.333-21.333zm21.333 92.535c-11.782 0-21.333 9.553-21.333 21.333 0 11.785 9.551 21.333 21.333 21.333h303.012c11.78 0 21.333-9.549 21.333-21.333 0-11.78-9.553-21.333-21.333-21.333H284.841z" />
                                                    <path
                                                        d="M325.731 43.151h15.654c1.387-.283 2.823-.432 4.294-.432s2.907.149 4.294.432H654.22c37.875 0 68.74 30.919 68.74 68.78v649.225c0 37.858-30.865 68.779-68.74 68.779H218.073c-37.873 0-68.741-30.921-68.741-68.779V212.754c0-.922.058-1.831.172-2.722.466-11.074 4.843-22.22 13.986-31.371L285.747 56.306c11.501-11.236 26.231-15.109 39.984-13.155zM193.673 208.819L315.626 86.765c.943-.899 1.808-1.238 2.577-1.366.895-.149 1.968-.049 3.028.39 1.055.437 1.833 1.1 2.312 1.78.366.52.73 1.278.803 2.512v70.051c0 .256.004.511.013.765v38.38c0 9.981-8.243 18.205-18.173 18.205H197.149c-1.328 0-2.141-.36-2.728-.777-.686-.486-1.363-1.285-1.806-2.354s-.529-2.115-.384-2.956c.124-.722.455-1.588 1.441-2.575zm173.34-123.001v3.525c.009.399.013.8.013 1.202v108.731c0 33.512-27.312 60.872-60.839 60.872L192 260.151v501.005c0 14.327 11.799 26.112 26.074 26.112h436.147c14.276 0 26.074-11.785 26.074-26.112V111.931c0-14.33-11.797-26.113-26.074-26.113H367.013z" />
                                                    <path
                                                        d="M777.485 128.521c-11.785 0-21.333 9.551-21.333 21.333s9.549 21.333 21.333 21.333h28.442c14.276 0 26.074 11.783 26.074 26.113v715.254c0 14.332-11.797 26.112-26.074 26.112H369.78c-14.275 0-26.074-11.785-26.074-26.112v-28.075c0-11.78-9.551-21.333-21.333-21.333s-21.333 9.553-21.333 21.333v28.075c0 37.862 30.868 68.779 68.741 68.779h436.147c37.875 0 68.74-30.916 68.74-68.779V197.3c0-37.861-30.865-68.78-68.74-68.78h-28.442z" />
                                                </svg>
                                            </span>
                                            <span class="intentdata"> <b class="intentno">
                                                    @if (isset($onlineAdmission))
                                                        {{ $onlineAdmission[0]->total }}
                                                    @endif
                                                </b> Online Admission Applicants</span>
                                        </a>
                                    </div>
                                @endif
                                @if (Auth::guard('admin')->user()->admin_role == 1 || Auth::guard('admin')->user()->admin_role == 18)
                                    <div class="col-md-3 {{ $cursor_prevent }}">
                                        <a href="{{ route('hostelList') }}" class="intentbtn pinkcolor">
                                            <span class="intenticon">
                                                <svg fill="#000000" version="1.1" id="Layer_1"
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                                    viewBox="0 0 290.625 290.625" xml:space="preserve">
                                                    <g>
                                                        <g>
                                                            <path d="M281.25,278.906L281.25,278.906V2.344h-9.375v70.313h-5.55c0.525-1.472,0.862-3.038,0.862-4.688
       c0-7.753-6.309-14.063-14.063-14.063H79.688c-7.753,0-14.063,6.309-14.063,14.063c0,1.65,0.338,3.216,0.863,4.688H60.07
       c0.53-1.472,0.867-3.038,0.867-4.688c0-7.753-6.309-14.063-14.063-14.063H37.5c-7.753,0-14.063,6.309-14.063,14.063
       c0,1.65,0.337,3.216,0.862,4.688h-5.55V2.344H9.375v276.563H0v9.375h290.625v-9.375H281.25z M79.688,63.281h173.438
       c2.587,0,4.688,2.105,4.688,4.688s-2.1,4.688-4.688,4.688H79.688C77.1,72.656,75,70.552,75,67.969S77.1,63.281,79.688,63.281z
       M192.187,91.406v-9.375h51.563v9.375H192.187z M243.75,100.781v9.375h-51.563v-9.375H243.75z M243.75,232.031v9.375h-51.563
       v-9.375H243.75z M37.501,63.281h9.375c2.588,0,4.688,2.105,4.688,4.688s-2.1,4.688-4.688,4.688H37.5
       c-2.588-0.001-4.687-2.105-4.687-4.688S34.913,63.281,37.501,63.281z M18.75,82.031H37.5h9.375h32.813h103.125v9.375H18.75V82.031
       z M159.375,203.906v-7.584l-3.581-1.791h27.019v9.375H159.375z M182.813,213.281v9.375h-23.438v-9.375H182.813z M18.749,100.781
       h164.063v84.375h-28.124v-17.33c0-8.33-4.139-16.064-11.067-20.681c-4.092-2.733-8.864-4.177-13.791-4.177h-7.936
       c10.332-0.011,18.731-8.416,18.731-18.749c0-10.341-8.409-18.75-18.75-18.75s-18.75,8.409-18.75,18.75
       c0,10.334,8.399,18.738,18.731,18.749h-7.936c-4.927,0-9.694,1.444-13.786,4.177c-6.933,4.617-11.072,12.347-11.072,20.681v17.33
       h-9.375c-7.753,0-14.063,6.309-14.063,14.063c0,1.65,0.338,3.216,0.863,4.688H60.07c0.53-1.472,0.867-3.037,0.867-4.688
       c0-7.753-6.309-14.063-14.063-14.063H37.5c-7.753,0-14.063,6.309-14.063,14.063c0,1.65,0.337,3.216,0.862,4.688h-5.55V100.781z
       M121.875,199.073l-23.438,5.859v48.192c0,1.289-1.055,2.344-2.344,2.344s-2.344-1.055-2.344-2.344v-51.009l18.75-9.375v-26.334
       h-9.375v20.541l-4.688,2.344v-21.464c0-5.189,2.578-10.008,6.895-12.881c2.55-1.706,5.517-2.602,8.587-2.602h15.914
       c3.066,0,6.037,0.895,8.587,2.602c4.313,2.873,6.891,7.688,6.891,12.881v21.464l-4.688-2.344v-20.541h-9.375v26.334l18.75,9.375
       v51.009c0,1.289-1.055,2.344-2.344,2.344s-2.344-1.055-2.344-2.344v-48.192L121.875,199.073z M135.939,212.253v10.402h-0.001
       h-28.125v-10.402l14.063-3.516L135.939,212.253z M112.5,124.219c0-5.17,4.205-9.375,9.375-9.375s9.375,4.205,9.375,9.375
       s-4.205,9.375-9.375,9.375S112.5,129.388,112.5,124.219z M37.5,213.281h9.375h32.813h4.688v9.375h-0.001H18.75v-9.375H37.5z
       M32.813,199.219c0-2.587,2.1-4.688,4.688-4.688h9.375c2.588,0,4.688,2.1,4.688,4.688s-2.1,4.688-4.688,4.688H37.5
       C34.912,203.906,32.813,201.805,32.813,199.219z M84.375,196.322v7.584h-4.688c-2.588,0-4.688-2.1-4.688-4.688
       c0-2.588,2.1-4.688,4.688-4.688h8.269L84.375,196.322z M182.814,278.906H18.75v-46.875h65.625v21.094
       c0,6.464,5.255,11.719,11.719,11.719s11.719-5.255,11.719-11.719v-21.094h28.125v21.094c0,6.464,5.255,11.719,11.719,11.719
       c6.464,0,11.719-5.255,11.719-11.719v-21.094h23.438V278.906z M192.187,250.781h51.563v9.375h-51.563V250.781z M243.751,278.906
       h-51.563v-9.375h51.563V278.906z M243.751,222.656h-51.563v-9.375h51.563V222.656z M243.751,203.906h-51.563v-9.375h51.563
       V203.906z M243.751,185.156h-51.563v-9.375h51.563V185.156z M243.751,166.406h-51.563v-9.375h51.563V166.406z M243.751,147.656
       h-51.563v-9.375h51.563V147.656z M243.751,128.906h-51.563v-9.375h51.563V128.906z M271.875,278.906h-18.75v-46.875h18.75V278.906
       z M271.875,222.656h-18.75v-9.375h18.75V222.656z M271.875,203.906h-18.75V100.781h18.75V203.906z M271.875,91.406h-18.75v-9.375
       h18.75V91.406z" />
                                                        </g>
                                                    </g>
                                                </svg>
                                            </span>
                                            <span class="intentdata"> <b class="intentno">
                                                    @if (isset($hostelapplicant))
                                                        {{ $hostelapplicant[0]->total }}
                                                    @endif
                                                </b> Hostel Applicants</span>
                                        </a>
                                    </div>
                                @endif
                                @if (isset($is_directorate) && $is_directorate != 3 && $is_college != 1 && Auth::guard('admin')->user()->admin_role != 17)
                                    <div class="col-md-3 {{ $cursor_prevent }}">
                                        <a href="{{ route('admin_player') }}" class="intentbtn yellowcolor">
                                            <span class="intenticon">
                                                <svg fill="#000000" viewBox="0 0 32 32" version="1.1"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <title>users</title>
                                                    <path
                                                        d="M16 21.416c-5.035 0.022-9.243 3.537-10.326 8.247l-0.014 0.072c-0.018 0.080-0.029 0.172-0.029 0.266 0 0.69 0.56 1.25 1.25 1.25 0.596 0 1.095-0.418 1.22-0.976l0.002-0.008c0.825-3.658 4.047-6.35 7.897-6.35s7.073 2.692 7.887 6.297l0.010 0.054c0.127 0.566 0.625 0.982 1.221 0.982 0.69 0 1.25-0.559 1.25-1.25 0-0.095-0.011-0.187-0.031-0.276l0.002 0.008c-1.098-4.78-5.305-8.295-10.337-8.316h-0.002zM9.164 11.102c0 0 0 0 0 0 2.858 0 5.176-2.317 5.176-5.176s-2.317-5.176-5.176-5.176c-2.858 0-5.176 2.317-5.176 5.176v0c0.004 2.857 2.319 5.172 5.175 5.176h0zM9.164 3.25c0 0 0 0 0 0 1.478 0 2.676 1.198 2.676 2.676s-1.198 2.676-2.676 2.676c-1.478 0-2.676-1.198-2.676-2.676v0c0.002-1.477 1.199-2.674 2.676-2.676h0zM22.926 11.102c2.858 0 5.176-2.317 5.176-5.176s-2.317-5.176-5.176-5.176c-2.858 0-5.176 2.317-5.176 5.176v0c0.004 2.857 2.319 5.172 5.175 5.176h0zM22.926 3.25c1.478 0 2.676 1.198 2.676 2.676s-1.198 2.676-2.676 2.676c-1.478 0-2.676-1.198-2.676-2.676v0c0.002-1.477 1.199-2.674 2.676-2.676h0zM31.311 19.734c-0.864-4.111-4.46-7.154-8.767-7.154-0.395 0-0.784 0.026-1.165 0.075l0.045-0.005c-0.93-2.116-3.007-3.568-5.424-3.568-2.414 0-4.49 1.448-5.407 3.524l-0.015 0.038c-0.266-0.034-0.58-0.057-0.898-0.063l-0.009-0c-4.33 0.019-7.948 3.041-8.881 7.090l-0.012 0.062c-0.018 0.080-0.029 0.173-0.029 0.268 0 0.691 0.56 1.251 1.251 1.251 0.596 0 1.094-0.417 1.22-0.975l0.002-0.008c0.684-2.981 3.309-5.174 6.448-5.186h0.001c0.144 0 0.282 0.020 0.423 0.029 0.056 3.218 2.679 5.805 5.905 5.805 3.224 0 5.845-2.584 5.905-5.794l0-0.006c0.171-0.013 0.339-0.035 0.514-0.035 3.14 0.012 5.765 2.204 6.442 5.14l0.009 0.045c0.126 0.567 0.625 0.984 1.221 0.984 0.69 0 1.249-0.559 1.249-1.249 0-0.094-0.010-0.186-0.030-0.274l0.002 0.008zM16 18.416c-0 0-0 0-0.001 0-1.887 0-3.417-1.53-3.417-3.417s1.53-3.417 3.417-3.417c1.887 0 3.417 1.53 3.417 3.417 0 0 0 0 0 0.001v-0c-0.003 1.886-1.53 3.413-3.416 3.416h-0z">
                                                    </path>
                                                </svg>
                                            </span>
                                            <span class="intentdata"> <b class="intentno">
                                                    @if (isset($players))
                                                        {{ $players[0]->total }}
                                                    @endif
                                                </b> Players</span>
                                        </a>
                                    </div>
                                    <div class="col-md-3 {{ $cursor_prevent }}">
                                        <a href="{{ route('admin_coach') }}" class="intentbtn greencolor">
                                            <span class="intenticon">
                                                <svg fill="#000000" version="1.1" id="Capa_1"
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                                    viewBox="0 0 477.365 477.365" xml:space="preserve">
                                                    <g>
                                                        <g>
                                                            <path d="M235.02,34.518c29.4,0,53.245,30.583,53.245,68.309c0,37.722-23.845,68.307-53.245,68.307
       c-29.404,0-53.238-30.584-53.238-68.307C181.781,65.101,205.615,34.518,235.02,34.518z" />
                                                            <path d="M67.996,369.691c71.659,0,61.618-25.443,99.787-22.102c8.195,0.729,14.593,0.922,19.603,0.773
       c10.832-0.288,22.145-8.415,28.136-10.331c3.234-1.029,7.049-1.827,11.185-1.827c12.052,0,27.451,6.035,35.49,6.035
       c2.34,0,4.003-0.453,5.186-1.114c2.589-1.414-1.495-6.348-6.865-8.929c-5.365-2.592-16.45-6.64-24.771-9.04
       c-8.321-2.404-21.961-4.509-30.471-4.697c-8.501-0.176-17.191,0.71-19.418,2.017c-2.224,1.286-8.975,1.595-15.065,0.673
       l-11.047-1.679l-36.159-4.686l-21.43-7.373c0,0-34.22-14.443-68.998-9.093c-23.875,7.009-30.713,17.564-32.376,27.943
       C-1.473,340.293-3.652,369.691,67.996,369.691z" />
                                                            <path d="M109.465,300.311l22.228-18.69v27.884l32.015,4.315V183.392c-18.745,9.123-42.839,22.053-56.916,34.277
       c-15.677,13.619-52.219,46.819-78.415,73.588C28.369,291.258,66.269,289.379,109.465,300.311z" />
                                                            <path d="M131.692,369.179v73.668h32.015v-85.65c-9.543,0.152-14.854,2.881-22.285,6.969
       C138.455,365.784,135.315,367.499,131.692,369.179z" />
                                                            <path d="M185.917,181.486v132.409c5.298-1.354,13.354-1.562,17.138-1.562l2.4,0.016c2.695,0.061,5.843,0.305,9.139,0.674V197.767
       l-14.727-16.28H185.917z" />
                                                            <path d="M430.452,346.02c8.984,0,17.088,0.89,24.072,2.637l1.379,0.489c1.903,0.877,3.582,1.694,5.27,2.517l-28.781-39.405
       l-59.286,42.486C381.188,352.443,405.986,346.02,430.452,346.02z" />
                                                            <path d="M327.938,365.476v-40.663l36.154,29.41l65.798-50.001c0,0-72.386-88.39-86.42-101.444
       c-6.917-6.436-22.502-13.966-37.137-20.554v186.233l7.826-1.187L327.938,365.476z" />
                                                            <path d="M327.938,413.224c-5.246-2.088-10.873-3.054-19.949-2.256c-0.569,0.048-1.099,0.076-1.655,0.124v31.755h21.596v-29.623
       H327.938z" />
                                                            <path d="M271.169,348.78c-4.909,2.705-11.786,2.128-20.418,0.305v19.043c6.664-1.118,13.602-1.915,18.891-2.031l2.429-0.024
       c2.669,0,7.486,0.108,12.055,0.693V173.181h-13.954l-19.42,26.062v121.325c5.189,1.867,10.043,3.803,13.196,5.326
       c5.458,2.629,11.918,7.998,12.087,14.414C276.134,343.888,274.311,347.045,271.169,348.78z" />
                                                            <path d="M284.125,411.26c-7.915-1.058-15.381-4.66-20.999-7.497c-2.4-1.211-4.645-2.353-5.951-2.777
       c-2.184-0.689-4.324-1.058-6.424-1.238v43.1h19.42h13.954V411.26z" />
                                                            <path d="M450.084,355.156c-32.135-8.039-79.675,6.692-79.675,6.692l-21.437,7.37l-36.154,4.692l-11.045,1.671
       c-6.104,0.918-12.852,0.613-15.067-0.677c-2.221-1.295-10.908-2.192-19.416-2.008c-8.521,0.192-22.149,2.284-30.471,4.692
       c-8.324,2.4-19.412,6.456-24.774,9.045c-5.364,2.593-9.454,7.514-6.863,8.94c1.178,0.649,2.841,1.103,5.18,1.103
       c8.041,0,23.443-6.023,35.491-6.023c4.14,0,7.951,0.781,11.185,1.823c5.996,1.907,17.305,10.034,28.137,10.327
       c5.014,0.145,11.413-0.057,19.601-0.773c38.179-3.342,28.128,22.101,99.788,22.101c71.657,0,74.579-30.168,72.322-44.197
       C475.207,369.555,471.536,365.03,450.084,355.156z" />
                                                            <path
                                                                d="M211.236,381.024c0.966-0.461,2.128-0.966,3.358-1.495v-30.812c-0.839,0.408-1.695,0.829-2.623,1.306
       c-6.412,3.234-15.198,7.679-24.359,7.935c-0.525,0.013-1.132,0.005-1.687,0.013v84.861h13.95h14.728v-37.097
       c-4.176,0.236-7.703-0.241-10.596-1.836c-3.136-1.723-4.957-4.889-4.861-8.464C199.312,389.027,205.762,383.661,211.236,381.024z" />
                                                        </g>
                                                    </g>
                                                </svg>
                                            </span>
                                            <span class="intentdata"> <b class="intentno">
                                                    @if (isset($coaches))
                                                        {{ $coaches[0]->total }}
                                                    @endif
                                                </b> Coaches</span>
                                        </a>
                                    </div>
                                @endif

                                @if (Auth::guard('admin')->user()->admin_role == 1 || Auth::guard('admin')->user()->admin_role == 18)
                                    <div class="col-md-3 {{ $cursor_prevent }}">
                                        <a href="{{ route('coaching_camp_list') }}" class="intentbtn box5color">
                                            <span class="intenticon">
                                                {{-- <svg fill="#000000" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
									<title>users</title>
									<path d="M16 21.416c-5.035 0.022-9.243 3.537-10.326 8.247l-0.014 0.072c-0.018 0.080-0.029 0.172-0.029 0.266 0 0.69 0.56 1.25 1.25 1.25 0.596 0 1.095-0.418 1.22-0.976l0.002-0.008c0.825-3.658 4.047-6.35 7.897-6.35s7.073 2.692 7.887 6.297l0.010 0.054c0.127 0.566 0.625 0.982 1.221 0.982 0.69 0 1.25-0.559 1.25-1.25 0-0.095-0.011-0.187-0.031-0.276l0.002 0.008c-1.098-4.78-5.305-8.295-10.337-8.316h-0.002zM9.164 11.102c0 0 0 0 0 0 2.858 0 5.176-2.317 5.176-5.176s-2.317-5.176-5.176-5.176c-2.858 0-5.176 2.317-5.176 5.176v0c0.004 2.857 2.319 5.172 5.175 5.176h0zM9.164 3.25c0 0 0 0 0 0 1.478 0 2.676 1.198 2.676 2.676s-1.198 2.676-2.676 2.676c-1.478 0-2.676-1.198-2.676-2.676v0c0.002-1.477 1.199-2.674 2.676-2.676h0zM22.926 11.102c2.858 0 5.176-2.317 5.176-5.176s-2.317-5.176-5.176-5.176c-2.858 0-5.176 2.317-5.176 5.176v0c0.004 2.857 2.319 5.172 5.175 5.176h0zM22.926 3.25c1.478 0 2.676 1.198 2.676 2.676s-1.198 2.676-2.676 2.676c-1.478 0-2.676-1.198-2.676-2.676v0c0.002-1.477 1.199-2.674 2.676-2.676h0zM31.311 19.734c-0.864-4.111-4.46-7.154-8.767-7.154-0.395 0-0.784 0.026-1.165 0.075l0.045-0.005c-0.93-2.116-3.007-3.568-5.424-3.568-2.414 0-4.49 1.448-5.407 3.524l-0.015 0.038c-0.266-0.034-0.58-0.057-0.898-0.063l-0.009-0c-4.33 0.019-7.948 3.041-8.881 7.090l-0.012 0.062c-0.018 0.080-0.029 0.173-0.029 0.268 0 0.691 0.56 1.251 1.251 1.251 0.596 0 1.094-0.417 1.22-0.975l0.002-0.008c0.684-2.981 3.309-5.174 6.448-5.186h0.001c0.144 0 0.282 0.020 0.423 0.029 0.056 3.218 2.679 5.805 5.905 5.805 3.224 0 5.845-2.584 5.905-5.794l0-0.006c0.171-0.013 0.339-0.035 0.514-0.035 3.14 0.012 5.765 2.204 6.442 5.14l0.009 0.045c0.126 0.567 0.625 0.984 1.221 0.984 0.69 0 1.249-0.559 1.249-1.249 0-0.094-0.010-0.186-0.030-0.274l0.002 0.008zM16 18.416c-0 0-0 0-0.001 0-1.887 0-3.417-1.53-3.417-3.417s1.53-3.417 3.417-3.417c1.887 0 3.417 1.53 3.417 3.417 0 0 0 0 0 0.001v-0c-0.003 1.886-1.53 3.413-3.416 3.416h-0z"></path>
								</svg> --}}
                                                <svg fill="#000000" version="1.1" id="Layer_1"
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 256 190"
                                                    enable-background="new 0 0 256 190" xml:space="preserve">
                                                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round"
                                                        stroke-linejoin="round"></g>
                                                    <g id="SVGRepo_iconCarrier">
                                                        <path
                                                            d="M48.12,27.903C48.12,13.564,59.592,2,74.023,2c14.339,0,25.903,11.564,25.903,25.903 C99.834,42.335,88.27,53.806,74.023,53.806C59.684,53.806,48.12,42.242,48.12,27.903z M191,139h-47V97c0-20.461-17.881-37-38-37H43 C20.912,60,1.99,79.14,2,98v77c-0.026,8.533,6.001,12.989,12,13c6.014,0.011,12-4.445,12-13v-75h8v88h78v-88h8l0.081,50.37 c-0.053,8.729,5.342,12.446,10.919,12.63h60C207.363,163,207.363,139,191,139z M203.288,67.115c8.152,0,14.788,6.636,14.788,14.788 S211.44,96.69,203.288,96.69c-8.152,0-14.788-6.636-14.788-14.788S195.136,67.115,203.288,67.115z M203.288,55.154 c-14.747,0-26.708,11.961-26.708,26.708s11.961,26.708,26.708,26.708c6.431,0,12.33-2.253,17-6.103 c0.778,0.492,1.72,0.819,2.785,0.819c2.826,0,5.161-2.253,5.161-5.161c0-1.393-0.573-2.622-1.393-3.564 c2.048-3.769,3.277-8.152,3.277-12.739C229.955,67.115,217.994,55.154,203.288,55.154z M225.367,93.577 c-0.737-0.369-1.475-0.614-2.376-0.614c-2.826,0-5.161,2.253-5.161,5.161c0,1.229,0.451,2.335,1.106,3.195 c-4.301,3.482-9.749,5.612-15.73,5.612c-13.804,0-25.028-11.224-25.028-25.028s11.265-25.028,25.028-25.028 c13.764,0,25.028,11.224,25.028,25.028C228.316,86.04,227.251,90.095,225.367,93.577z M203.329,43.643 c-8.152,0-15.689,2.417-21.997,6.595c-1.27-1.27-2.99-2.007-4.916-2.007c-3.891,0-6.964,3.154-6.964,6.964 c0,1.843,0.737,3.482,1.843,4.711c-4.424,6.185-7.087,13.723-7.087,21.874c0,21.055,17.532,38.136,39.079,38.136 s39.079-17.122,39.079-38.136C242.408,60.725,224.916,43.643,203.329,43.643z M203.329,118.196c-20.645,0-37.44-16.385-37.44-36.498 c0-7.701,2.499-14.829,6.718-20.768c1.106,0.778,2.417,1.147,3.891,1.147c3.891,0,6.964-3.154,6.964-6.964 c0-1.311-0.369-2.622-1.065-3.646c5.981-3.932,13.19-6.267,20.932-6.267c20.645,0,37.44,16.385,37.44,36.498 C240.769,101.892,223.974,118.196,203.329,118.196z M254,55.932c0-4.875-3.973-8.848-8.848-8.848c-2.007,0-3.81,0.655-5.325,1.761 c-9.094-9.585-22.079-15.566-36.498-15.566c-27.445,0-49.77,21.751-49.77,48.5s22.325,48.5,49.77,48.5s49.77-21.751,49.77-48.5 c0-6.431-1.27-12.576-3.605-18.147C252.157,62.077,254,59.209,254,55.932z M251.46,81.78c0,25.848-21.587,46.861-48.131,46.861 s-48.09-21.055-48.09-46.861s21.587-46.861,48.09-46.861c13.886,0,26.462,5.817,35.269,14.992c-1.434,1.557-2.335,3.646-2.335,5.981 c0,4.875,3.973,8.848,8.848,8.848c0.983,0,1.925-0.164,2.826-0.492C250.231,69.696,251.46,75.594,251.46,81.78z">
                                                        </path>
                                                    </g>
                                                </svg>
                                            </span>
                                            <span class="intentdata"> <b class="intentno">
                                                    @if (isset($coaching_camp))
                                                        {{ $coaching_camp[0]->total }}
                                                    @endif
                                                </b> Coaching Camps</span>
                                        </a>
                                    </div>
                                @endif

                    </div>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    <p style="text-align: right;"><button type="button" data-print="modal"
                            class="btn btn-sm  btn-outline-primary ms-2 rounded-pill"
                            onclick="PrintIt( '{{ Auth::guard('admin')->user()->username }}')"><span
                                class="icons icon-printer"></span></button>
                    </p>
                    <div class="table-responsive" id="prodiv">
                        <table id="dataTable" class="table table-striped table-hover table-bordered">
                            <thead>
                                <tr>
                                    <!-- <th align="center">S.No.</th> -->
                                    <th>Module Name</th>
                                    <th align="center">Total Application Received</th>
                                    <th align="center">Applications Forwarded</th>
                                    <th align="center">Applications Pending</th>
                                    <th align="center">Applications Accepted</th>
                                    <th align="center">Applications Rejected</th>
                                    <!-- <th align="center">Total</th> -->
                                </tr>
                            </thead>
                            <tbody>
                                @if (isset($is_recruitment) && (($is_recruitment == 1 && $is_directorate == 1) || $is_directorate == 3) && Auth::guard('admin')->user()->admin_role != 17)
                                    <tr class="{{ $cursor_prevent }}">
                                        @foreach ($direct as $key => $list)
                                            <!-- <td align="center">1</td> -->
                                            <td>Direct Recruitment </td>
                                            <td align="center"><a
                                                    href="{{ route('direct_rect') }}">{{ $list->total }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('direct_rect_spe', 3) }}">{{ $list->total_app_forward }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('direct_rect_spe', 0) }}">{{ $list->total_pending }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('direct_rect_spe', 1) }}">{{ $list->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('direct_rect_spe', 2) }}">{{ $list->total_rejected }}</a>
                                            </td>
                                            <!-- <td align="center">&nbsp;</td> -->
                                        @endforeach
                                    </tr>
                                @endif
                                @if (isset($is_directorate) && $is_directorate == 1 && $is_rso == 0 && $is_college == 0)
                                    <tr class="{{ $cursor_prevent }}">
                                        @foreach ($financial as $key => $list)
                                            <!-- <td align="center">2</td> -->
                                            <td>Financial Assistance</td>
                                            <td align="center"><a
                                                    href="{{ route('financial_assis') }}">{{ $list->total }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('financial_assis_spe', 3) }}">{{ $list->total_app_forward }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('financial_assis_spe', 0) }}">{{ $list->total_pending }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('financial_assis_spe', 1) }}">{{ $list->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('financial_assis_spe', 2) }}">{{ $list->total_rejected }}</a>
                                            </td>
                                            <!-- <td align="center">&nbsp;</td> -->
                                        @endforeach
                                    </tr>
                                    <tr class="{{ $cursor_prevent }}">
                                        @foreach ($monthly as $key => $list)
                                            <!-- <td align="center">3</td> -->
                                            <td>Monthly Pension</td>
                                            <td align="center"><a
                                                    href="{{ route('monthly_pension') }}">{{ $list->total }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('monthly_pension_spe', 3) }}">{{ $list->total_app_forward }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('monthly_pension_spe', 0) }}">{{ $list->total_pending }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('monthly_pension_spe', 1) }}">{{ $list->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('monthly_pension_spe', 2) }}">{{ $list->total_rejected }}</a>
                                            </td>
                                            <!-- <td align="center">&nbsp;</td> -->
                                        @endforeach
                                    </tr>
                                    <tr class="{{ $cursor_prevent }}">
                                        @foreach ($laxman as $key => $list)
                                            <!-- <td align="center">4</td> -->
                                            <td>Nomination for Laxman Award</td>
                                            <td align="center"><a href="{{ route('laxman') }}">{{ $list->total }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('laxman_spe', 3) }}">{{ $list->total_app_forward }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('laxman_spe', 0) }}">{{ $list->total_pending }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('laxman_spe', 1) }}">{{ $list->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('laxman_spe', 2) }}">{{ $list->total_rejected }}</a>
                                            </td>
                                            <!-- <td align="center">&nbsp;</td> -->
                                        @endforeach
                                    </tr>
                                    <tr class="{{ $cursor_prevent }}">
                                        @foreach ($ranilaxmibai as $key => $list)
                                            <!-- <td align="center">5</td> -->
                                            <td>Nomination for Rani Laxmibai Award</td>
                                            <td align="center"><a href="{{ route('laxmibai') }}">{{ $list->total }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('laxmibai_spe', 3) }}">{{ $list->total_app_forward }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('laxmibai_spe', 0) }}">{{ $list->total_pending }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('laxmibai_spe', 1) }}">{{ $list->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('laxmibai_spe', 2) }}">{{ $list->total_rejected }}</a>
                                            </td>
                                            <!-- <td align="center">&nbsp;</td> -->
                                        @endforeach
                                    </tr>
                                    @endif @if ((isset($is_directorate) && ($is_directorate == 1 && $is_rso == 1)) || (isset($is_associate) && $is_associate == 1))
                                        <tr class="{{ $cursor_prevent }}">
                                            @foreach ($position as $key => $list)
                                                <!-- <td align="center">6</td> -->
                                                <td>Nomination for Prize Money</td>
                                                <td align="center"><a
                                                        href="{{ route('award') }}">{{ $list->total }}</a>
                                                </td>
                                                <td align="center"><a
                                                        href="{{ route('award_spe', 3) }}">{{ $list->total_app_forward }}</a>
                                                </td>
                                                <td align="center"><a
                                                        href="{{ route('award_spe', 0) }}">{{ $list->total_pending }}</a>
                                                </td>
                                                <td align="center"><a
                                                        href="{{ route('award_spe', 1) }}">{{ $list->total_accepted }}</a>
                                                </td>
                                                <td align="center"><a
                                                        href="{{ route('award_spe', 2) }}">{{ $list->total_rejected }}</a>
                                                </td>
                                                <!-- <td align="center">&nbsp;</td> -->
                                            @endforeach
                                        </tr>
                                        <tr class="{{ $cursor_prevent }}">
                                            @foreach ($eklavya as $key => $list)
                                                <!-- <td align="center">3</td> -->
                                                <td>Eklavya Krida Kosh</td>
                                                <td align="center"><a
                                                        href="{{ route('eklavya_krida') }}">{{ $list->total }}</a>
                                                </td>
                                                <td align="center"><a href="{{ route('eklavya_krida_spe', 3) }}">0</a>
                                                </td>
                                                <td align="center"><a
                                                        href="{{ route('eklavya_krida_spe', 0) }}">{{ $list->total_pending }}</a>
                                                </td>
                                                <td align="center"><a
                                                        href="{{ route('eklavya_krida_spe', 1) }}">{{ $list->total_accepted }}</a>
                                                </td>
                                                <td align="center"><a
                                                        href="{{ route('eklavya_krida_spe', 2) }}">{{ $list->total_rejected }}</a>
                                                </td>
                                                <!-- <td align="center">&nbsp;</td> -->
                                            @endforeach
                                        </tr>
                                    @endif



                                    <tr class="{{ $cursor_prevent }}">
                                        @if (isset($onlineAdmission) &&
                                                isset($is_directorate) &&
                                                $is_directorate == 1 &&
                                                ($is_college == 1 || Auth::guard('admin')->user()->admin_role == 1 || Auth::guard('admin')->user()->admin_role == 18))
                                            <!-- <td align="center">3</td> -->
                                            <td>Online Admission Applicant</td>
                                            <td align="center"><a
                                                    href="{{ route('collegeadmindashboard') }}">{{ $onlineAdmission[0]->total }}</a>
                                            </td>
                                            <td align="center"><a href="">0</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('filterpending') }}">{{ $onlineAdmission[0]->total_pending }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('filteraccepted') }}">{{ $onlineAdmission[0]->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('filterrejected') }}">{{ $onlineAdmission[0]->total_rejected }}</a>
                                            </td>
                                            <!-- <td align="center">&nbsp;</td> -->
                                        @endif
                                    </tr>

                                    <tr class="{{ $cursor_prevent }}">
                                        @if (Auth::guard('admin')->user()->admin_role == 1 || Auth::guard('admin')->user()->admin_role == 18)
                                            <!-- <td align="center">3</td> -->
                                            <td>Hostel Applicant</td>
                                            <td align="center"><a
                                                    href="{{ route('hostelList') }}">{{ $hostelapplicant[0]->total }}</a>
                                            </td>
                                            <td align="center"><a href="">0</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('hostelList', 3) }}">{{ $hostelapplicant[0]->total_pending }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('hostelList', 1) }}">{{ $hostelapplicant[0]->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('hostelList', 2) }}">{{ $hostelapplicant[0]->total_rejected }}</a>
                                            </td>
                                            <!-- <td align="center">&nbsp;</td> -->
                                        @endif
                                    </tr>

                                    <tr class="{{ $cursor_prevent }}">
                                        @if (isset($is_directorate) && $is_directorate != 3 && $is_college != 1 && Auth::guard('admin')->user()->admin_role != 17)
                                            <td>Player Applicant</td>
                                            <td align="center"><a
                                                    href="{{ route('admin_player') }}">{{ $players[0]->total }}</a>
                                            </td>
                                            <td align="center"><a href="{{ route('admin_player') }}">0</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('admin_player_status', 1) }}">{{ $players[0]->total_pending }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('admin_player_status', 3) }}">{{ $players[0]->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('admin_player_status', 2) }}">{{ $players[0]->total_rejected }}</a>
                                            </td>
                                        @endif
                                    </tr>

                                    <tr class="{{ $cursor_prevent }}">
                                        @if (isset($is_directorate) && $is_directorate != 3 && $is_college != 1)
                                            <!-- <td align="center">3</td> -->
                                            <td>Coach Applicant</td>
                                            <td align="center"><a
                                                    href="{{ route('admin_coach') }}">{{ $coaches[0]->total }}</a>
                                            </td>
                                            <td align="center"><a href="{{ route('admin_coach') }}">0</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('admin_coach_status', 1) }}">{{ $coaches[0]->total_pending }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('admin_coach_status', 3) }}">{{ $coaches[0]->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="{{ route('admin_coach_status', 2) }}">{{ $coaches[0]->total_rejected }}</a>
                                            </td>
                                            <!-- <td align="center">&nbsp;</td> -->
                                        @endif


                                    </tr>

                                    @if (Auth::guard('admin')->user()->admin_role == 1 || Auth::guard('admin')->user()->admin_role == 18)
                                        <tr class="{{ $cursor_prevent }}">

                                            <!-- <td align="center">3</td> -->
                                            <td>Coaching Camps</td>
                                            <td align="center"><a
                                                    href="{{ route('coaching_camp_list') }}">{{ $coaching_camp[0]->total }}</a>
                                            </td>
                                            <td align="center"><a href="#">0</a>
                                            </td>
                                            <td align="center"><a href="#">{{ $coaching_camp[0]->total }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="#">{{ $coaching_camp[0]->total_accepted }}</a>
                                            </td>
                                            <td align="center"><a
                                                    href="#">{{ $coaching_camp[0]->total_rejected }}</a>
                                            </td>
                                            <!-- <td align="center">&nbsp;</td> -->



                                        </tr>
                                    @endif
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>




@endsection

@section('modal_content')

    @if (count($broadcast_list) > 0)
        <div class="modal fade" id="basicExampleModal" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Latest Broadcasts</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div id="newpost" class="hidden Dglow"> <i class="fa fa-close new_close"></i>
                            <h5 class="modal-title" id="exampleModalLongTitle"></h5>


                            <div id="view_broadcast_special"></div>
                            <div class="row" id="attachmentList" style="
                margin: 0;
            ">

                            </div>





                        </div>


                        <ol class="olcards">
                            @foreach ($broadcast_list as $item)
                                <li>
                                    <div class="content">
                                        <div class="title">{{ $item->subject }}</div>
                                        <div class="text">{!! substr_replace($item->message, '...', 100) !!}<br>
                                            <small>{{ admin_name($item->created_by) }} |
                                                {{ dmyhi($item->created_on) }}</small>
                                        </div>
                                        {{-- <div class="icon" style="right: 51px;">
				 </div> --}}
                                        <div class="icon"><i class="fa fa-eye" id="button" aria-hidden="true"
                                                onclick="changeBroadcastStatus({{ $item->idd }})"></i> </div>
                                    </div>
                                </li>
                            @endforeach
                            {{-- <li>
            <div class="content">
              <div class="title">Lorem Ipsum</div>
              <div class="text">Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium, voluptatem.<br>
                <small>Admin | 14-12-2023 6:PM</small> </div>
				<div class="icon" style="right: 51px;">
				<i class="fa fa-eye" id="button" aria-hidden="true"></i> </div>
				<div class="icon"><i class="fa fa-picture-o"></i> </div>
            </div>
          </li>
          <li>
            <div class="content">
              <div class="title">Lorem Ipsum</div>
              <div class="text">Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium, voluptatem.<br>
                <small>Admin | 14-12-2023 6:PM</small> </div>
				<div class="icon" style="right: 51px;">
				<i class="fa fa-eye" id="button" aria-hidden="true"></i> </div>
				<div class="icon"><i class="fa fa-picture-o"></i> </div>
            </div>
          </li>
          <li>
            <div class="content">
              <div class="title">Lorem Ipsum</div>
              <div class="text">Lorem ipsum dolor sit amet consectetur adipisicing elit. Accusantium, voluptatem.<br>
                <small>Admin | 14-12-2023 6:PM</small> </div>
				<div class="icon" style="right: 51px;">
				<i class="fa fa-eye" id="button" aria-hidden="true"></i> </div>
				<div class="icon"><i class="fa fa-picture-o"></i> </div>
            </div>
          </li> --}}
                        </ol>
                    </div>
                </div>
            </div>
        </div>
        </div>


        <script>
            $("#button").click(function() {
                $("#newpost").toggleClass("hidden");
            });
            $(".new_close").click(function() {
                $("#newpost").addClass("hidden");;
            });
        </script>
    @endif
@endsection
@push('custom-scripts')
    <script type="text/javascript">
        function changeBroadcastStatus(id) {

            var actionUrl = ajaxUrl + "/admin/changeBroadcastStatus/" + id;

            $.ajax({
                type: "GET",
                url: actionUrl,


                success: function(res) {

                    $("#exampleModalLongTitle").empty();
                    $("#view_broadcast_special").empty();
                    $("#attachmentList").empty();
                    if (res.error == false) {



                        $("#exampleModalLongTitle").append(res.data.subject);
                        $("#view_broadcast_special").append(res.data.message);

                        if (res.attachment) {

                            res.attachment.forEach(item => {
                                console.log(item)
                                $("#attachmentList").append(`<div class="col-md-1 mx-3">
                             <a href="{{ asset('mail/attachment/${item.attachment}') }}" download><img src="{{ asset('assets_admin/images/paperclip.png') }}" alt="" srcset=""></a>
                         <div>`);
                            });

                        }

                        $("#newpost").removeClass("hidden");


                    } else {
                        error(res.msg);
                    }
                },
            });
        }

        $(document).ready(function() {
            $('#basicExampleModal').modal('show');
        });
        $(function() {
            var table = $('.yajra-datatable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('projectlist') }}",
                columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex'
                }, {
                    data: 'fullname',
                    name: 'fullname'
                }, {
                    data: 'project_id',
                    name: 'project_id'
                }, {
                    data: 'project_name',
                    name: 'project_name'
                }, {
                    data: 'application_date',
                    name: 'application_date'
                }, {
                    data: 'current_status',
                    name: 'current_status',
                    orderable: false,
                    searchable: false
                }, {
                    data: 'view',
                    name: 'view',
                    orderable: false,
                    searchable: false
                }, ]
            });
        });
    </script>

    @if (count(announcement()) > 0)
        {{-- <marquee behavior="scroll" direction="left" onmouseover="this.stop();" onmouseout="this.start();" class="marquee">
    @foreach (announcement() as $item)
     <strong>&#x2022;</strong> {{ $item}}
    @endforeach
</marquee> --}}


        <div class="chatbox chatbox22 chatbox--tray">
            <div class="chatbox__title">
                <h5><a href="javascript:void()">Announcements</a></h5>
                <button class="chatbox__title__close"> <span>
                        <svg viewBox="0 0 12 12" width="12px" height="12px">
                            <line stroke="#FFFFFF" x1="11.75" y1="0.25" x2="0.25" y2="11.75"></line>
                            <line stroke="#FFFFFF" x1="11.75" y1="11.75" x2="0.25" y2="0.25"></line>
                        </svg>
                    </span> </button>
            </div>



            <div class="chatbox__body">
                @foreach (announcement() as $item)
                    <div class="chatbox__body__message chatbox__body__message--left">
                        <div class="chatbox_timing">
                            <ul>
                                <li><a href="#" style="
                color: #d64d92;
            "><i
                                            class="fa fa-calendar"></i> {{ dmy($item->start_date) }}</a></li>
                                {{-- <li><a href="#"><i class="fa fa-clock-o"></i> 7:00 PM</a></a></li> --}}
                            </ul>
                        </div>
                        <div class="ul_section_full">
                            <ul class="ul_msg">
                                <li class="mb-2"><strong
                                        style="
                color: #d64d92;
            ">{{ admin_name($item->created_by) }}</strong>
                                </li>

                                <li><strong>{{ $item->announcement }} </strong></li>
                            </ul>
                        </div>
                    </div>
                @endforeach
                {{-- <div class="chatbox__body__message chatbox__body__message--left">
        <div class="chatbox_timing">
          <ul>
            <li><a href="#"><i class="fa fa-calendar"></i> 22/11/2018</a></li>
            <li><a href="#"><i class="fa fa-clock-o"></i> 7:00 PM</a></a></li>
          </ul>
        </div>
        <div class="ul_section_full">
          <ul class="ul_msg">
            <li><strong>Person Name</strong></li>
            <li>Lorem ipsum dolor sit amet, consectetur adipiscing elit. </li>
          </ul>
        </div>
      </div>
  <div class="chatbox__body__message chatbox__body__message--left">
        <div class="chatbox_timing">
          <ul>
            <li><a href="#"><i class="fa fa-calendar"></i> 22/11/2018</a></li>
            <li><a href="#"><i class="fa fa-clock-o"></i> 7:00 PM</a></a></li>
          </ul>
        </div>
        <div class="ul_section_full">
          <ul class="ul_msg">
            <li><strong>Person Name</strong></li>
            <li>Lorem ipsum dolor sit amet, consectetur adipiscing elit. </li>
          </ul>
        </div>
      </div>
 --}}

            </div>

            <div class="panel-footer text-end">
                <a href="{{ route('announcement') }}" class="btn bt_bg btn-sm" id="btn-chat"> View All</a>
            </div>
        </div>
    @endif





    <script>
        (function($) {
            $(document).ready(function() {
                var $chatbox = $('.chatbox'),
                    $chatboxTitle = $('.chatbox__title'),
                    $chatboxTitleClose = $('.chatbox__title__close'),
                    $chatboxCredentials = $('.chatbox__credentials');
                $chatboxTitle.on('click', function() {
                    $chatbox.toggleClass('chatbox--tray');
                });
                $chatboxTitleClose.on('click', function(e) {
                    e.stopPropagation();
                    $chatbox.addClass('chatbox--closed');
                });
                $chatbox.on('transitionend', function() {
                    if ($chatbox.hasClass('chatbox--closed')) $chatbox.remove();
                });

            });
        })(jQuery);
    </script>


@endpush
