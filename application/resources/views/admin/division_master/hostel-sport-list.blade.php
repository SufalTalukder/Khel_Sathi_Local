...
<div class="row">
	<div class="col-md-12">
		<div class="pageheader" id="menu-margin">
			<h4 class="mb-0">Create Sport For Hostel</h4>
		</div>
		<div class="card mb-3">
			<div class="card-body">
				<form action="{{ route('hostelsaveSport') }}" class="needs-validation" method="post" novalidate>
					@csrf
					<div class="row">
						<div class="col-md-3">
						...
						<table id="dataTable" class="table table-bordred table-hover bg-white datatable">
							<thead>
								<tr>
                                    ...
                                    <th width="20%">Trial Male Date</th>
                                    <th width="20%">Trial Female Date</th>
							...
							<tbody>
								...
								<tr>
                                    ...
                                    <td>@if($sport->trial_male_date){{ dmy($sport->trial_male_date) }} @else NA @endif</td>
                                    <td>@if($sport->trial_female_date){{ dmy($sport->trial_female_date) }} @else NA @endif</td>
									<td class="text-center">
										<a class="btn btn-sm btn-dark pointer bt role_manager_id" href="javascript:void(0)" data-toggle="modal" data-target="#exampleModal" data-id="{{$sport->id}}" data-trialmaledate="{{$sport->trial_male_date}}"  data-trialfemaledate="{{$sport->trial_female_date}}" data-name="{{$sport->name}}"data-subsport="{{sub_sport_type($sport->id)}}" data-gender="{{$sport->gender}}">
	...
	<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<form action="{{ route('hostelupdateSport') }}" class="needs-validation" method="post" autocomplete="off">
					@csrf
					<div class="modal-body">
                        ...
                        <div class="form-group">
							<label for="gender">Trial Male Date *</label><br>
							<input type="date" name="trial_male_date" value="" id="trial_male_date" class="form-control" required>
                            @if ($errors->has('trial_male_date'))
                            <span class="error_mess">{{ $errors->first('trial_male_date') }}</span> @endif
						</div>

                        <div class="form-group">
							<label for="gender">Trial Female Date *</label><br>
							<input type="date" name="trial_female_date" value="" id="trial_female_date" class="form-control" required>
                            @if ($errors->has('trial_female_date'))
                            <span class="error_mess">{{ $errors->first('trial_female_date') }}</span> @endif
						</div>
	...
	<script type="text/javascript">
		...
		$('.role_manager_id').click(function() {
            ...
            var trial_male_date = $(this).data('trialmaledate');
            
            var trial_female_date = $(this).data('trialfemaledate');
			var subsport = $(this).data('subsport');
			$('#sport_id').val(sport_id);
			$('#trial_male_date').val(trial_male_date);
            $('#trial_female_date').val(trial_female_date);
            	$('#name').val(name);
		...
		function reloadContainer() {
			...
			window.location.href = "admin/hostel-sport-list";
		}
		...
		function sportStatus(id) {
			$.ajax({
				type: "GET",
				url: ajaxUrl + "/admin/hostel-sportStatus/" + id,
				dataType: "text",
