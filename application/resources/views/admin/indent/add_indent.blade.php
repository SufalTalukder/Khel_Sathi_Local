...
@section( 'content' )
					...
					<div class="pageheader" id="menu-margin">
						<div class="row">
							<div class="col-md-8">
								...
								<h4 class="mb-0">Raise Indent</h4>
							</div>
							<div class="col-md-4">
							<a href="{{route('indentList')}}" class="btn btn-primary" style="float: right;">Details of Raise Indent</a>
							</div>
						</div>
					</div>
					...
					<div class="card mb-3">
						...
						<div class="card-body">
							<form action="{{ route('addIndent') }}" id="preregistration" class="needs-validation" enctype="multipart/form-data" method="post" autocomplete="off" novalidate>
								<div class="row">
									...
									<div class="row mt-3">
										<div class="col-md-3">
											<div class="form-group">
												<label for="AMC_required">Raise Indent For <span class="text-danger">*</span></label>
												<div class="form-control">
													<input type="radio" @if(isset($list) && $list->AMC_required == 1) checked @endif class="form-check-input" onchange=apply_for(this.value); required name="indent_for" id="status" value="1" >
													<label for="status" class="form-check-label">Section</label>
													<input type="radio" @if(isset($list) && $list->AMC_required == 2) checked @endif class="form-check-input" onchange=apply_for(this.value); name="indent_for" id="status" value="2" >
													<label for="status2" class="form-check-label">Individual</label>
													<div class="invalid-feedback">
													Please Check Raise Indent For?.
												</div>
												@if ($errors->has('indent_for'))
												<span class="error_mess">{{ $errors->first('indent_for') }}</span> @endif
												</div>
											</div>
										</div>
											...
											<div class="form-group">
												<label for="employee_id">Employee <span class="text-danger">*</span></label>
													<select  id="employee_id" class="form-control" name="employee_id"  required>
														...
														<option  value="{{$type->id}}" @if(isset($list) && $list[0]->employee_id == $type->id ) selected @endif {{ old('vendor') === $type->id ? 'selected' : '' }}>{{$type->employee_name}}</option>
														@endforeach
													</select>
												<div class="invalid-feedback">
											...
											<div class="form-group">
												<label for="section_id">Section<span class="text-danger">*</span></label>
													<select  id="section_id" class="form-control" name="section_id"  required>
														...
														<option  value="{{$type->id}}" @if(isset($list) && $list[0]->section_id == $type->id ) selected @endif {{ old('vendor') === $type->id ? 'selected' : '' }}>{{$type->name_hn}}</option>
														@endforeach
													</select>
												<div class="invalid-feedback">
												...
												<span class="error_mess">{{ $errors->first('section_id') }}</span> @endif
											</div>
										</div>
										<div class="col-md-3">
											<div class="form-group">
												<label for="order_date">Date of Indent <span class="text-danger">*</span></label>
												<input type="date" class="form-control"   min="<?php echo date("Y-m-d"); ?>" id="indent_date" name="indent_date" @if(isset($list)) value="{{ $list[0]->indent_date }}" @else  value="{{ old('indent_date') }}" @endif placeholder="Enter Date of Indent" required="">
												<div class="invalid-feedback">
													Please provide Date of Indent.
												</div>
												@if ($errors->has('indent_date'))
												<span class="error_mess">{{ $errors->first('indent_date') }}</span> @endif
											</div>
										</div>
									</div>
												...
												<table  class="table" id="paperdetails">
													...
													<tbody>
														...
														<tr>
															...
															<td>
															<button type="button" id="addmore" class="btn btn-info "><i class="fa fa-plus"></i> </button>
															</td>
														</tr>
														@endif
													</tbody>
...
$('#employee_div').hide();
		...
		$('#addmore').on('click', function () {
			...
			$('#paperdetails').find('tr:last').after("<tr><td>" + itemtype.html() + "</td><td>" + catid.html()+ "</td><td>" + sub_cat.html() + "</td><td>" + item_id + "</td><td>" + unit + "</td><td>" + quantity + "</td><td>" + remove + "</td></tr>");
		});
