				...
				<div class="col-md-12">
					<div class="card">
						<div class="card-header">
							<div class="row">
								<div class="col-md-10">
									<h5> Issue Items for Approved Indents</h5>
								</div>
							</div>
						</div>
						<div class="card-body">
							...
							<form action="{{ route('save_indent_issue') }}"   enctype="multipart/form-data" method="post" autocomplete="off" >
							@csrf
								<div class="col-lg-12" id="issued_btn">
									<div class="row">
										<div class="col-lg-4">
											<label>Remark<span class="text-danger">*</span></label>
											<input name="remark" required class="form-control" type="text" id="remark">
										</div>
										<div class="col-lg-4">
										...
										<div class="col-lg-2">
											<label>&nbsp; </label>
											<div><input class="btn btn-success form-group col-sm-12" id="issuebtn" name="submit" type="submit" value="Issue"></div>
										</div>
									</div>
								</div>
									...
									<table  id="dataTable" class="table table-bordred table-hover bg-white datatable" >
										<thead>
											<tr>
												...
												<th width="20%">Item Quantity Requested</th>
												<th width="20%">Item Quantity Approved</th>
												<th width="20%">Item Quantity In Stock</th>
												<th width="10%">Added By</th>
												<th width="20%">Issue Item Quantity</th>
										...
										<tbody>
										 @foreach($list as $key=>$item)
											<tr>
												...
												<td>{{ stock_item($item->item_id,$item->item_type_id) }}</td>
												<td>{{ rsoName($item->added_by) }}</td>
												<td><input   name="data[PurchaseIndentIssue][{{$key}}][issue_quantity]" value="0" min="0" max="{{($item->approved_quantity)}}" class="form-control issueq" type="number" id="PurchaseEntryRecevied"></td>
				...
				$(document).on('click', '.che', function () {                      
				 ...
				 $('#issuebtn').click(function () {                        
                       ...
                       if (numberOfChecked == 0) {
                            ...
                            if(k > 1){
                                error("Issue Quantity should be more than 0 !");
                                flag++; 
								return false;
                            }
                        }
                        ...
                        if ($('#issue_date').val() == "") {
                            error("Please Select Date of Issuance.");
                            flag++;
