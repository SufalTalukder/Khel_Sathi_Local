				...
				<div class="col-md-12">
					<div class="card">
						<div class="card-header">
							<div class="row">
								<div class="col-md-10">
									<h5> Indents Pending to Process</h5>
								</div>
								<!-- <div class="col-md-2">
									...
									<table  id="dataTable" class="table table-bordred table-hover bg-white datatable" >
										...
										<tbody>
											...
											<tr>
												...
												<td class="text-center">{{ dmy($item->indent_date) }}</td>
												
												<td>{{ rsoName($item->added_by) }}</td>
												<td class="text-center">
													<a href="javascript:void(0);" class="btn btn-danger btn-sm" onclick="indents_pending({{$item->indent_no}})">Proceed </i></a>
												</td>
...
@push( 'custom-scripts' )
	...
	<script>
		...
		function app_status(val,id){
            if(val==2){
				$('#approve_quantity'+id).removeAttr('required');
			}else{
				$('#approve_quantity'+id).prop('required',true);
			}
        }
