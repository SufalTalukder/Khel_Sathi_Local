			...
			<div class="card-body">
				...
				<div class="table-responsive">
					<table id="dataTable" class="table table-bordred table-hover bg-white datatable">
						...
						<tbody>
							@foreach($draft_mail as $key=>$item)
							<tr>
								...
								<td>{{ $item->subject }}</td>
                                   ...
                                   $message =  mb_strimwidth($item->message, 0, 100, "...")
                                   @endphp
								<td >{{  $message }}</td>

								<td>{{ dmyHi($item->created_on)}}</td>
                                <td><a href="{{ route('mail_compose',$item->id) }}" class="btn btn-outline-warning rounded-pill"  >Edit</a>
                                </td>
							</tr>
							@endforeach
						</tbody>
