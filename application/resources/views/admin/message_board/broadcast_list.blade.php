...
.comme:hover {
		...
		<div class="col-md-10">
			...
			<div class="col-md-2">
			<a href="{{ route('send_broadcast') }}" class="w-100 mb-0 btn btn-primary btn-sm ">
					...
					<table id="dataTable" class="table table-bordred table-hover bg-white datatable">
						...
						<tbody>
							...
							<tr>
								...
								{{-- <td ><div class="comme" >{!! $item->message!!}</div></td> --}}
                              <td >  {{admin_name($item->created_by)}}</td>

								<td>{{ dmyHi($item->created_on)}}</td>
									<td>
									<button class="btn btn-outline-primary rounded-pill" onclick="view_broadcast({{$item->id}})" ><i class="fa fa-eye"></i></button>

									@if($item->draft != 2)
											<button class="btn btn-outline-success rounded-pill" onclick="warning_broadcast({{$item->id}})" ><i class="fa fa-check"></i></button>
									@else

	                          	<button class="btn btn-outline-danger rounded-pill"  ><i class="fa fa-ban" aria-hidden="true"></i></button>
