...
<div class="row">
	<div class="col-md-12">
		...
		<div class="card">
			...
			<div class="card-body">
				<form action="{{route('hosteladmintrialListtwo')}}" method="post">
					@csrf
					<div class="row">
						...
						<div class="mb-3 col-md-2">
							...
							<select class="form-control"  name="sport" id="sport" onchange="sportgender(this.value)" data-gender="@if(isset($filterData)){{$filterData['gender']}}@endif"    data-subsport="@if(isset($filterData)){{$filterData['subSport']}}@endif" data-assignskill="@if(isset($filterData)){{$filterData['assign_skill'] ?? ''}}@endif" >
                        ...
                        <div class="mb-3 col-md-3" id="subsport_div" @if(empty($subSportList) || $subSportList->isEmpty()) style="display:none;" @endif>


							<label>Name of The Sub Sport</label>
							<select class="form-control"  id="subtype" name="subsport">
								@if(!empty($subSportList) && $subSportList->isNotEmpty())
								<option value="">Select Sub Sport</option>
								@foreach($subSportList as $ss)
								<option value="{{$ss->id}}" @if(isset($filterData) && $filterData['subSport'] == $ss->id) selected @endif>{{$ss->sub_type}}</option>
								@endforeach
								@endif
							</select>
                        ...
                        {{-- Skill field: shown when sport has sub-sports but no sub-sport filter is selected.
                             Lets admin designate which skill/position to use when filling data for
                             players who registered without a sub-sport. --}}
                        <div class="mb-3 col-md-3" id="skill_div" style="display:none;">
                            <label>Skill <small class="text-muted">(for players without sub-sport)</small></label>
                            <select class="form-control" id="assign_skill_select" name="assign_skill">
...
@csrf
		...
		@if(isset($applicant->application_no)) <?php $trial = hosteltrialData($applicant->application_no, 2);?>
		@if(isset($trial))
		...
		<td valign="top">
            ...
            <td><input type="hidden" name="application_no" value="{{$applicant->application_no}}">
                ...
                <input type="hidden" name="subsport_id" value="{{$filterData['subSport'] ?: ($filterData['assign_skill'] ?? '')}}">
            ...
            "></td>
                      @if ($trialType != 25)
                      <td><input type="number"  class="stop" data-id="{{$applicant->id}}" name="physical_total_mark" readonly max="50" id="phy{{$applicant->id}}" required @if(isset($trial) && $trial->physical_total_mark != '') value="{{($trial->physical_total_mark)}}" @endif style="
                width: 50px;
                font-weight: 400;
                background: none;
                border: 2px solid #868686;
            "></td>
                      <td><input type="checkbox" @if(isset($trial) && $trial->physical_total_mark != '') disabled checked @endif id="check{{$applicant->id}}" onChange="myfunction({{$applicant->id}})" style="
                width: 50px;
                font-weight: 400;
                background: none;
                border: 1px solid #cdcdcd;
...
{{-- change on basis of subSport and gender --}}
                    ...
                    @if ($trialType != 25)
	                  @if ($trialType == 1)
						...
						<form action="{{route('hostelhockeytrialList')}}" id="hockey_{{$applicant->id}}" class="needs-validation hockeyData"  novalidate method="post">
							...
							@if(isset($applicant->application_no)) <?php $trialhockey = hosteltrialhockeyData($applicant->application_no, 2);?>
						...
						<td>
                            ...
                            <input type="hidden" name="trial_type" value="2">
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
							<input type="number" name="hit_mark"   class="stop-{{$applicant->id}}" @if(isset($trialhockey) && $trialhockey->hit_mark != '') readonly @endif  value="@if(isset($trialhockey)){{$trialhockey->hit_mark}}@endif"  oninput="hockeyskillTotal({{$applicant->id}})" id="hit_mark{{$applicant->id}}" style="
						width: 50px;
						font-weight: 400;
						background: none;
						border: 1px solid #cdcdcd;
						...
						<td><input type="number"   class="stop-{{$applicant->id}}" name="test_score_mark" readonly  @if(isset($trialhockey) && $trialhockey->test_score_mark != '') value="{{$trialhockey->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
						width: 50px;
						font-weight: 400;
						background: none;
						border: 2px solid #868686;
						...
						<td><input type="number"   class="stop-{{$applicant->id}}"    name="sport_test_mark" readonly @if(isset($trialhockey) && $trialhockey->sport_test_mark != '') value="{{$trialhockey->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
						width: 50px;
						font-weight: 400;
						background: none;
						border: 2px solid #868686;
						...
						<td><input type="number"   class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialhockey) && $trialhockey->total_obtain_mark != '') value="{{$trialhockey->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 2px solid #868686;
						...
						<td><input type="checkbox"   class="stop-{{$applicant->id}}" id="checkhockey{{$applicant->id}}" @if(isset($trialhockey) && $trialhockey->total_obtain_mark != '') checked disabled @endif   onChange="hockeyData({{$applicant->id}})"  {{$applicant->id}}  style="
						width: 50px;
						font-weight: 400;
						background: none;
						border: 1px solid #cdcdcd;
	...
	@elseif ($trialType == 2)
					...
					<form action="{{route('hostelhockeykeepertrialList')}}" id="hockeykeeper_{{$applicant->id}}" class="needs-validation hockeykeeperData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialhockeykeeper = hosteltrialhockeykeeperData($applicant->application_no, 2);?>
					...
					<td>
                        ...
                        <input type="hidden" name="trial_type" value="2">
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<input type="number" name="kick_mark"   class="stop-{{$applicant->id}}"  @if(isset($trialhockeykeeper) && $trialhockeykeeper->kick_mark != '') readonly @endif  value="@if(isset($trialhockeykeeper)){{$trialhockeykeeper->kick_mark}}@endif"  oninput="hockeykeeperskillTotal({{$applicant->id}})" id="kick_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
					...
					<td><input type="number"   class="stop-{{$applicant->id}}" name="test_score_mark" readonly   @if(isset($trialhockeykeeper) && $trialhockeykeeper->test_score_mark != '') value="{{$trialhockeykeeper->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number"   class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialhockeykeeper) && $trialhockeykeeper->sport_test_mark != '') value="{{$trialhockeykeeper->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number"   class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialhockeykeeper) && $trialhockeykeeper->total_obtain_mark != '') value="{{$trialhockeykeeper->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
		            ...
		            @elseif ($trialType == 3)
							...
							<form action="{{route('hostelbadmintontrialList')}}" id="badmin_{{$applicant->id}}" class="needs-validation badminData"  novalidate method="post">
								...
								@if(isset($applicant->application_no)) <?php $trialbadminton = hosteltrialBadmintonData($applicant->application_no , 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
							...
							<td><input type="number"   class="stop-{{$applicant->id}}" name="drop_mark" @if(isset($trialbadminton))  value="{{$trialbadminton->drop_mark}}" @endif  id="drop_mark{{$applicant->id}}"  oninput="badskillTotal({{$applicant->id}})"  style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 1px solid #cdcdcd;
							...
							<td><input type="number"   class="stop-{{$applicant->id}}" name="backhand_mark" @if(isset($trialbadminton))  value="{{$trialbadminton->backhand_mark}}" @endif  id="backhand_mark{{$applicant->id}}"  oninput="badskillTotal({{$applicant->id}})"  style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 1px solid #cdcdcd;
							...
							<td><input type="number"   class="stop-{{$applicant->id}}" name="test_score_mark" readonly  @if(isset($trialbadminton)) value="{{ $trialbadminton->test_score_mark != '' ? $trialbadminton->test_score_mark : (($trialbadminton->high_double_service_mark??0)+($trialbadminton->smash_mark??0)+($trialbadminton->drop_mark??0)+($trialbadminton->backhand_mark??0)) }}" @endif  id="test_score_mark{{$applicant->id}}"    style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 2px solid #868686;
							...
							<td><input type="number"   class="stop-{{$applicant->id}}" name="game_technique"   @if(isset($trialbadminton))    value="{{$trialbadminton->game_technique}}"  @endif  oninput="mainTotal({{$applicant->id}})" id="game_technique{{$applicant->id}}" style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 1px solid #cdcdcd;
							...
							<td><input type="number"   class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialbadminton)) value="{{ $trialbadminton->sport_test_mark != '' ? $trialbadminton->sport_test_mark : ((($trialbadminton->high_double_service_mark??0)+($trialbadminton->smash_mark??0)+($trialbadminton->drop_mark??0)+($trialbadminton->backhand_mark??0))+($trialbadminton->game_technique??0)) }}" @endif  id="sport_test_mark{{$applicant->id}}" style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 2px solid #868686;
							...
							<td><input type="number"   class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialbadminton)) value="{{ $trialbadminton->total_obtain_mark != '' ? $trialbadminton->total_obtain_mark : (($trial->physical_total_mark??0)+((($trialbadminton->high_double_service_mark??0)+($trialbadminton->smash_mark??0)+($trialbadminton->drop_mark??0)+($trialbadminton->backhand_mark??0))+($trialbadminton->game_technique??0))) }}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 2px solid #868686;
							...
							<td><input type="text"   class="stop-{{$applicant->id}}" name="remark" @if(isset($trialbadminton))  value="{{$trialbadminton->remark}}" @endif  id="remark{{$applicant->id}}" style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 1px solid #cdcdcd;
							...
							<form action="{{route('hostelvolleyBalltrialList')}}" id="volleyball_{{$applicant->id}}" class="needs-validation volleyballData"  novalidate method="post">
								...
								@if(isset($applicant->application_no)) <?php $trialvolleyball = hosteltrialvolleyballData($applicant->application_no ,2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
							...
							<td><input type="number"   class="stop-{{$applicant->id}}" name="test_score_mark" readonly  @if(isset($trialvolleyball) && $trialvolleyball->test_score_mark != '') value="{{$trialvolleyball->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 2px solid #868686;
							...
							<td><input type="number"   class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialvolleyball) && $trialvolleyball->sport_test_mark != '') value="{{$trialvolleyball->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 2px solid #868686;
							...
							<td><input type="number"   class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialvolleyball) && $trialvolleyball->total_obtain_mark != '') value="{{$trialvolleyball->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
							width: 50px;
							font-weight: 400;
							background: none;
							border: 2px solid #868686;
			...
			<form action="{{route('hostelkustitrialList')}}" id="kusti_{{$applicant->id}}" class="needs-validation kustiData"  novalidate method="post">
				...
				@if(isset($applicant->application_no)) <?php $trialkusti = hosteltrialkustiData($applicant->application_no ,2);?>
				@if(isset($trialkusti))
			...
			<td><input type="number"   class="stop-{{$applicant->id}}"  name="test_score_mark" readonly  @if(isset($trialkusti) && $trialkusti->test_score_mark != '') value="{{$trialkusti->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number"   class="stop-{{$applicant->id}}"  name="sport_test_mark" readonly @if(isset($trialkusti) && $trialkusti->sport_test_mark != '') value="{{$trialkusti->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number"   class="stop-{{$applicant->id}}"    name="total_obtain_mark" readonly @if(isset($trialkusti) && $trialkusti->total_obtain_mark != '') value="{{$trialkusti->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<form action="{{route('hostelswimmingtrialList')}}" id="swimming_{{$applicant->id}}" class="needs-validation swimmingData"  novalidate method="post">
				...
				@if(isset($applicant->application_no)) <?php $trialswimming = hosteltrialswimmingData($applicant->application_no , 2);?>
			...
			<td>
                ...
                <input type="hidden" name="trial_type" value="2">
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
				<input type="number" name="free_stroke_mark" class="stop-{{$applicant->id}}" @if(isset($trialswimming) && $trialswimming->free_stroke_mark != '') readonly @endif  value="@if(isset($trialswimming)){{$trialswimming->free_stroke_mark}}@endif"  oninput="swimmingskillTotal({{$applicant->id}})" id="free_stroke_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 1px solid #cdcdcd;
			...
			<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark" readonly   @if(isset($trialswimming) && $trialswimming->test_score_mark != '') value="{{$trialswimming->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialswimming) && $trialswimming->sport_test_mark != '') value="{{$trialswimming->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialswimming) && $trialswimming->total_obtain_mark != '') value="{{$trialswimming->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			@elseif ($trialType == 7)
					...
					<form action="{{route('hostelfootballkeepertrialList')}}" id="footballkeeper_{{$applicant->id}}" class="needs-validation footballkeeperData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialfootballkeeper = hosteltrialfootballkeeperData($applicant->application_no ,2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number"   class="stop-{{$applicant->id}}" name="test_score_mark" readonly  @if(isset($trialfootballkeeper) && $trialfootballkeeper->test_score_mark != '') value="{{$trialfootballkeeper->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number"   class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialfootballkeeper) && $trialfootballkeeper->sport_test_mark != '') value="{{$trialfootballkeeper->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number"   class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialfootballkeeper) && $trialfootballkeeper->total_obtain_mark != '') value="{{$trialfootballkeeper->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="text"   class="stop-{{$applicant->id}}" name="remark" @if(isset($trialfootballkeeper) && $trialfootballkeeper->remark != '') readonly @endif value="@if(isset($trialfootballkeeper)){{$trialfootballkeeper->remark}}@endif"  id="remark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
					...
					<form action="{{route('hostelfootballtrialList')}}" id="football_{{$applicant->id}}" class="needs-validation footballData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialfootball = hosteltrialfootballData($applicant->application_no ,2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number"   class="stop-{{$applicant->id}}" name="test_score_mark" readonly  @if(isset($trialfootball) && $trialfootball->test_score_mark != '') value="{{$trialfootball->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number"   class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialfootball) && $trialfootball->sport_test_mark != '') value="{{$trialfootball->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number"   class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialfootball) && $trialfootball->total_obtain_mark != '') value="{{$trialfootball->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="text"   class="stop-{{$applicant->id}}" name="remark" @if(isset($trialfootball) && $trialfootball->remark != '') readonly @endif value="@if(isset($trialfootball)){{$trialfootball->remark}}@endif"  id="remark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
					...
					<form action="{{route('hostelathleticsjumpertrialList')}}" id="athleticsjumper_{{$applicant->id}}" class="needs-validation athleticsjumperData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialathleticsjumper = hosteltrialathleticsjumperData($applicant->application_no , 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark" readonly  @if(isset($trialathleticsjumper) && $trialathleticsjumper->test_score_mark != '') value="{{$trialathleticsjumper->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialathleticsjumper) && $trialathleticsjumper->sport_test_mark != '') value="{{$trialathleticsjumper->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialathleticsjumper) && $trialathleticsjumper->total_obtain_mark != '') value="{{$trialathleticsjumper->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="text" class="stop-{{$applicant->id}}" name="remark" @if(isset($trialathleticsjumper) && $trialathleticsjumper->remark != '') readonly @endif value="@if(isset($trialathleticsjumper)){{$trialathleticsjumper->remark}}@endif"  id="remark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
				...
				<form action="{{route('hostelcricketbatsmantrialList')}}" id="cricketbatsman_{{$applicant->id}}" class="needs-validation cricketbatsmanData"  novalidate method="post">
					...
					@if(isset($applicant->application_no)) <?php $trialcricketbatsman = hosteltrialcricketbatsmanData($applicant->application_no, 2);?>
				...
				<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    readonly @if(isset($trialcricketbatsman) && $trialcricketbatsman->test_score_mark != '') value="{{$trialcricketbatsman->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
				width: 50px;
				font-weight: 400;
				background: none;
				border: 2px solid #868686;
				...
				<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialcricketbatsman) && $trialcricketbatsman->sport_test_mark != '') value="{{$trialcricketbatsman->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
				width: 50px;
				font-weight: 400;
				background: none;
				border: 2px solid #868686;
				...
				<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialcricketbatsman) && $trialcricketbatsman->total_obtain_mark != '') value="{{$trialcricketbatsman->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
				width: 50px;
				font-weight: 400;
				background: none;
				border: 2px solid #868686;
			...
			<form action="{{route('hostelcricketballertrialList')}}" id="cricketballer_{{$applicant->id}}" class="needs-validation cricketballerData"  novalidate method="post">
				...
				@if(isset($applicant->application_no)) <?php $trialcricketballer = hosteltrialcricketballerData($applicant->application_no ,2);?>
			...
			<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    readonly @if(isset($trialcricketballer) && $trialcricketballer->test_score_mark != '') value="{{$trialcricketballer->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialcricketballer) && $trialcricketballer->sport_test_mark != '') value="{{$trialcricketballer->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly  value="@if(isset($trialcricketballer)){{$trialcricketballer->total_obtain_mark}}@endif"  id="total_obtain_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<form action="{{route('hostelcricketkeepertrialList')}}" id="cricketkeeper_{{$applicant->id}}" class="needs-validation cricketkeeperData"  novalidate method="post">
				...
				@if(isset($applicant->application_no)) <?php $trialcricketkeeper = hosteltrialcricketkeeperData($applicant->application_no , 2);?>
			...
			<td>
                ...
                <input type="hidden" name="trial_type" value="2">
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
				<input type="number" class="stop-{{$applicant->id}}" name="stumping_mark" @if(isset($trialcricketkeeper) && $trialcricketkeeper->stumping_mark != '') readonly @endif value="@if(isset($trialcricketkeeper)){{$trialcricketkeeper->stumping_mark}}@endif" id="stumping_mark{{$applicant->id}}" oninput="cricketkeeperskillTotal({{$applicant->id}})" style="width: 50px; font-weight: 400; background: none; border: 1px solid #cdcdcd;">
			</td>
			...
			<td><input type="number" class="stop-{{$applicant->id}}" name="off_stumping_gathering_mark" @if(isset($trialcricketkeeper))  value="{{$trialcricketkeeper->off_stumping_gathering_mark}}" @endif  id="off_stumping_gathering_mark{{$applicant->id}}"  oninput="cricketkeeperskillTotal({{$applicant->id}})"  style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 1px solid #cdcdcd;
			...
			<td><input type="number" class="stop-{{$applicant->id}}"  name="on_stumping_gathering_mark" @if(isset($trialcricketkeeper))  value="{{$trialcricketkeeper->on_stumping_gathering_mark}}" @endif  id="on_stumping_gathering_mark{{$applicant->id}}"  oninput="cricketkeeperskillTotal({{$applicant->id}})"  style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 1px solid #cdcdcd;
			...
			<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    @if(isset($trialcricketkeeper))value="{{$trialcricketkeeper->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number" class="stop-{{$applicant->id}}" name="game_technique"   @if(isset($trialcricketkeeper))    value="{{$trialcricketkeeper->game_technique}}"  @endif  oninput="mainTotal({{$applicant->id}})" id="game_technique{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 1px solid #cdcdcd;
			...
			<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" @if(isset($trialcricketkeeper))    value="{{$trialcricketkeeper->sport_test_mark}}"  @endif  id="sport_test_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark"@if(isset($trialcricketkeeper))  value="{{$trialcricketkeeper->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="text" class="stop-{{$applicant->id}}" name="remark" @if(isset($trialcricketkeeper))  value="{{$trialcricketkeeper->remark}}" @endif  id="remark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 1px solid #cdcdcd;
			...
			<form action="{{route('hostelkabadditrialList')}}" id="kabaddi_{{$applicant->id}}" class="needs-validation kabaddiData"  novalidate method="post">
				...
				@if(isset($applicant->application_no)) <?php $trialkabaddi = hosteltrialkabaddiData($applicant->application_no, 2);?>
			...
			<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    readonly value="@if(isset($trialkabaddi)){{$trialkabaddi->test_score_mark}}@endif"  id="test_score_mark{{$applicant->id}}"    style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly value="@if(isset($trialkabaddi)){{$trialkabaddi->sport_test_mark}}@endif"  id="sport_test_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
			...
			<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly value="@if(isset($trialkabaddi)){{$trialkabaddi->total_obtain_mark}}@endif"  id="total_obtain_mark{{$applicant->id}}" style="
			width: 50px;
			font-weight: 400;
			background: none;
			border: 2px solid #868686;
        ...
        <form action="{{route('hosteljudotrialList')}}" id="judo_{{$applicant->id}}" class="needs-validation judoData"  novalidate method="post">
            ...
            @if(isset($applicant->application_no)) <?php $trialjudo = hosteltrialjudoData($applicant->application_no, 2);?>
            @if(isset($trialjudo))
        ...
        <td>
            ...
            <input type="hidden" name="trial_type" value="2">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="number" class="stop-{{$applicant->id}}" name="straight_work_throw_mark" @if(isset($trialjudo)) value="{{$trialjudo->straight_work_throw_mark}}" @endif id="straight_work_throw_mark{{$applicant->id}}"  oninput="judoskillTotal({{$applicant->id}})"  style="width: 50px; font-weight: 400; background: none; border: 1px solid #cdcdcd;">
        </td>
        <td><input type="number" class="stop-{{$applicant->id}}" name="hip_leg_hand_techniquec_mark" @if(isset($trialjudo))   value="{{$trialjudo->hip_leg_hand_techniquec_mark}}" @endif id="hip_leg_hand_techniquec_mark{{$applicant->id}}"  oninput="judoskillTotal({{$applicant->id}})"  style="
        width: 50px;
        font-weight: 400;
        background: none;
        border: 1px solid #cdcdcd;
        ...
        <td><input type="number" class="stop-{{$applicant->id}}" name="throw_count_mark" @if(isset($trialjudo))  value="{{$trialjudo->throw_count_mark}}" @endif  id="throw_count_mark{{$applicant->id}}"  oninput="judoskillTotal({{$applicant->id}})"  style="
        width: 50px;
        font-weight: 400;
        background: none;
        border: 1px solid #cdcdcd;
        ...
        <td><input type="number" class="stop-{{$applicant->id}}"  name="throw_combination_mark" @if(isset($trialjudo))  value="{{$trialjudo->throw_combination_mark}}" @endif  id="throw_combination_mark{{$applicant->id}}"  oninput="judoskillTotal({{$applicant->id}})"  style="
        width: 50px;
        font-weight: 400;
        background: none;
        border: 1px solid #cdcdcd;
        ...
        <td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    readonly value="@if(isset($trialjudo)){{ $trialjudo->test_score_mark != '' ? $trialjudo->test_score_mark : (($trialjudo->straight_work_throw_mark??0)+($trialjudo->hip_leg_hand_techniquec_mark??0)+($trialjudo->throw_count_mark??0)+($trialjudo->throw_combination_mark??0)) }}@endif"  id="test_score_mark{{$applicant->id}}"    style="
        width: 50px;
        font-weight: 400;
        background: none;
        border: 2px solid #868686;
        ...
        <td><input type="number" class="stop-{{$applicant->id}}" name="game_technique"   @if(isset($trialjudo))    value="{{$trialjudo->game_technique}}"  @endif  oninput="mainTotal({{$applicant->id}})" id="game_technique{{$applicant->id}}" style="
        width: 50px;
        font-weight: 400;
        background: none;
        border: 1px solid #cdcdcd;
        ...
        <td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly value="@if(isset($trialjudo)){{ $trialjudo->sport_test_mark != '' ? $trialjudo->sport_test_mark : ((($trialjudo->straight_work_throw_mark??0)+($trialjudo->hip_leg_hand_techniquec_mark??0)+($trialjudo->throw_count_mark??0)+($trialjudo->throw_combination_mark??0))+($trialjudo->game_technique??0)) }}@endif"  id="sport_test_mark{{$applicant->id}}" style="
        width: 50px;
        font-weight: 400;
        background: none;
        border: 2px solid #868686;
        ...
        <td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly value="@if(isset($trialjudo)){{ $trialjudo->total_obtain_mark != '' ? $trialjudo->total_obtain_mark : (($trial->physical_total_mark??0)+((($trialjudo->straight_work_throw_mark??0)+($trialjudo->hip_leg_hand_techniquec_mark??0)+($trialjudo->throw_count_mark??0)+($trialjudo->throw_combination_mark??0))+($trialjudo->game_technique??0))) }}@endif"  id="total_obtain_mark{{$applicant->id}}" style="
        width: 50px;
        font-weight: 400;
        background: none;
        border: 2px solid #868686;
        ...
        <td><input type="text" class="stop-{{$applicant->id}}" name="remark" @if(isset($trialjudo))  value="{{$trialjudo->remark}}" @endif  id="remark{{$applicant->id}}" style="
        width: 50px;
        font-weight: 400;
        background: none;
        border: 1px solid #cdcdcd;
        ...
        ">
					...
					<form action="{{route('hostelathleticsrunnertrialList')}}" id="athleticsrunner_{{$applicant->id}}" class="needs-validation athleticsrunnerData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialathleticsrunner = hosteltrialathleticsrunnerData($applicant->application_no, 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    readonly value="@if(isset($trialathleticsrunner)){{ $trialathleticsrunner->test_score_mark != '' ? $trialathleticsrunner->test_score_mark : (($trialathleticsrunner->stance_mark??0)+($trialathleticsrunner->start_mark??0)+($trialathleticsrunner->action_mark??0)+($trialathleticsrunner->finish_mark??0)) }}@endif"  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly value="@if(isset($trialathleticsrunner)){{ $trialathleticsrunner->sport_test_mark != '' ? $trialathleticsrunner->sport_test_mark : ((($trialathleticsrunner->stance_mark??0)+($trialathleticsrunner->start_mark??0)+($trialathleticsrunner->action_mark??0)+($trialathleticsrunner->finish_mark??0))+($trialathleticsrunner->game_technique??0)) }}@endif"  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly value="@if(isset($trialathleticsrunner)){{ $trialathleticsrunner->total_obtain_mark != '' ? $trialathleticsrunner->total_obtain_mark : (($trial->physical_total_mark??0)+((($trialathleticsrunner->stance_mark??0)+($trialathleticsrunner->start_mark??0)+($trialathleticsrunner->action_mark??0)+($trialathleticsrunner->finish_mark??0))+($trialathleticsrunner->game_technique??0))) }}@endif"  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
                    ...
                    <form action="{{route('hostelgymnasticboystrialList')}}" id="gymnasticboys_{{$applicant->id}}" class="needs-validation gymnasticboysData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialgymnasticboys = hosteltrialgymnasticboysData($applicant->application_no, 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark" readonly value="@if(isset($trialgymnasticboys)){{$trialgymnasticboys->test_score_mark}}@endif" id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly value="@if(isset($trialgymnasticboys)){{$trialgymnasticboys->sport_test_mark}}@endif" id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly value="@if(isset($trialgymnasticboys)){{$trialgymnasticboys->total_obtain_mark}}@endif" id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="text" class="stop-{{$applicant->id}}" name="remark" @if(isset($trialgymnasticboys) && $trialgymnasticboys->remark != '') readonly @endif  value="@if(isset($trialgymnasticboys)){{$trialgymnasticboys->remark}}@endif"  id="remark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
                    ...
                    <form action="{{route('hostelgymnasticgirlstrialList')}}" id="gymnasticgirls_{{$applicant->id}}" class="needs-validation gymnasticgirlsData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialgymnasticgirls = hosteltrialgymnasticgirlsData($applicant->application_no, 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark" readonly value="@if(isset($trialgymnasticgirls)){{$trialgymnasticgirls->test_score_mark}}@endif" id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly value="@if(isset($trialgymnasticgirls)){{$trialgymnasticgirls->sport_test_mark}}@endif" id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly value="@if(isset($trialgymnasticgirls)){{$trialgymnasticgirls->total_obtain_mark}}@endif" id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="text" class="stop-{{$applicant->id}}" name="remark" @if(isset($trialgymnasticgirls) && $trialgymnasticgirls->remark != '') readonly @endif  value="@if(isset($trialgymnasticgirls)){{$trialgymnasticgirls->remark}}@endif"  id="remark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
                    ...
                    <form action="{{route('hostelathleticsthrowertrialList')}}" id="athleticsthrower_{{$applicant->id}}" class="needs-validation athleticsthrowerData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialathleticsthrower = hosteltrialathleticsthrowerData($applicant->application_no, 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    readonly value="@if(isset($trialathleticsthrower)){{$trialathleticsthrower->test_score_mark}}@endif"  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly value="@if(isset($trialathleticsthrower)){{$trialathleticsthrower->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly value="@if(isset($trialathleticsthrower)){{$trialathleticsthrower->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
                    ...
                    <form action="{{route('hostelboxingtrialList')}}" id="boxing_{{$applicant->id}}" class="needs-validation boxingData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialboxing = hosteltrialboxingData($applicant->application_no, 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    readonly value="@if(isset($trialboxing)){{$trialboxing->test_score_mark}}@endif"  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly    value="@if(isset($trialboxing)){{$trialboxing->sport_test_mark}}@endif"  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly  value="@if(isset($trialboxing)){{$trialboxing->total_obtain_mark}}@endif"  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="checkbox" class="stop-{{$applicant->id}}"  id="checkboxing{{$applicant->id}}" @if(isset($trialboxing) && $trialboxing->total_obtain_mark != '') checked disabled @endif   onChange="boxingData({{$applicant->id}})"  {{$applicant->id}}  style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
                    ...
                    <form action="{{route('hostelbasketballtrialList')}}" id="basketball_{{$applicant->id}}" class="needs-validation basketballData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialbasketball = hosteltrialbasketballData($applicant->application_no, 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    readonly value="@if(isset($trialbasketball)){{$trialbasketball->test_score_mark}}@endif"  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly    value="@if(isset($trialbasketball)){{$trialbasketball->sport_test_mark}}@endif"  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly  value="@if(isset($trialbasketball)){{$trialbasketball->total_obtain_mark}}@endif"  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="checkbox" class="stop-{{$applicant->id}}"  id="checkbasketball{{$applicant->id}}" @if(isset($trialbasketball) && $trialbasketball->total_obtain_mark != '') checked disabled @endif   onChange="basketballData({{$applicant->id}})"  {{$applicant->id}}  style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
                    ...
                    <form action="{{route('hosteltabletennistrialList')}}" id="tabletennis_{{$applicant->id}}" class="needs-validation tabletennisData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialtabletennis = hosteltrialtabletennisData($applicant->application_no, 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" class="stop-{{$applicant->id}}"  name="service" @if(isset($trialtabletennis) && $trialtabletennis->service != '') readonly @endif  value="@if(isset($trialtabletennis)){{$trialtabletennis->service}}@endif"  id="service{{$applicant->id}}"  oninput="tabletennisskillTotal({{$applicant->id}})"  style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
					...
					<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark"    readonly value="@if(isset($trialtabletennis)){{$trialtabletennis->test_score_mark}}@endif"  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly    value="@if(isset($trialtabletennis)){{$trialtabletennis->sport_test_mark}}@endif"  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly  value="@if(isset($trialtabletennis)){{$trialtabletennis->total_obtain_mark}}@endif"  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="checkbox" class="stop-{{$applicant->id}}" id="checktabletennis{{$applicant->id}}" @if(isset($trialtabletennis) && $trialtabletennis->total_obtain_mark != '') checked disabled @endif   onChange="tabletennisData({{$applicant->id}})"  {{$applicant->id}}  style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
                    ...
                    <form action="{{route('hostelhandballtrialList')}}" id="handball_{{$applicant->id}}" class="needs-validation handballData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialhandball = hosteltrialhandballData($applicant->application_no, 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" class="stop-{{$applicant->id}}" name="test_score_mark" readonly  @if(isset($trialhandball) && $trialhandball->test_score_mark != '') value="{{$trialhandball->test_score_mark}}" @endif  id="test_score_mark{{$applicant->id}}"    style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="sport_test_mark" readonly @if(isset($trialhandball) && $trialhandball->sport_test_mark != '') value="{{$trialhandball->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number" class="stop-{{$applicant->id}}"   name="total_obtain_mark" readonly @if(isset($trialhandball) && $trialhandball->total_obtain_mark != '') value="{{$trialhandball->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="text" class="stop-{{$applicant->id}}" name="remark" @if(isset($trialhandball) && $trialhandball->remark != '') readonly @endif  value="@if(isset($trialhandball)){{$trialhandball->remark}}@endif"  id="remark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
                    ...
                    <form action="{{route('hostelarcherytrialList')}}" id="archery_{{$applicant->id}}" class="needs-validation archeryData"  novalidate method="post">
						...
						@if(isset($applicant->application_no)) <?php $trialarchery = hosteltrialarcheryData($applicant->application_no, 2);?>
				...
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
					...
					<td><input type="number" name="expansion" class="stop-{{$applicant->id}}" @if(isset($trialarchery) && $trialarchery->expansion != '') readonly @endif  value="@if(isset($trialarchery)){{$trialarchery->expansion}}@endif"  id="expansion{{$applicant->id}}"  oninput="archeryskillTotal({{$applicant->id}})"  style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
					...
					<td><input type="number"  name="driving" class="stop-{{$applicant->id}}" @if(isset($trialarchery) && $trialarchery->driving != '') readonly @endif  value="@if(isset($trialarchery)){{$trialarchery->driving}}@endif"  id="driving{{$applicant->id}}"  oninput="archeryskillTotal({{$applicant->id}})"  style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
                    ...
                    <td><input type="number"  name="anchoring" class="stop-{{$applicant->id}}" @if(isset($trialarchery) && $trialarchery->anchoring != '') readonly @endif  value="@if(isset($trialarchery)){{$trialarchery->anchoring}}@endif"  id="anchoring{{$applicant->id}}"  oninput="archeryskillTotal({{$applicant->id}})"  style="
                        width: 50px;
                        font-weight: 400;
                        background: none;
                        border: 1px solid #cdcdcd;
                    ...
                    <td><input type="number"  name="titan_hold" class="stop-{{$applicant->id}}" @if(isset($trialarchery) && $trialarchery->titan_hold != '') readonly @endif  value="@if(isset($trialarchery)){{$trialarchery->titan_hold}}@endif"  id="titan_hold{{$applicant->id}}"  oninput="archeryskillTotal({{$applicant->id}})"  style="
                        width: 50px;
                        font-weight: 400;
                        background: none;
                        border: 1px solid #cdcdcd;
                        ...
                        <td><input type="number"  name="aiming" class="stop-{{$applicant->id}}" @if(isset($trialarchery) && $trialarchery->aiming != '') readonly @endif  value="@if(isset($trialarchery)){{$trialarchery->aiming}}@endif"  id="aiming{{$applicant->id}}"  oninput="archeryskillTotal({{$applicant->id}})"  style="
                            width: 50px;
                            font-weight: 400;
                            background: none;
                            border: 1px solid #cdcdcd;
                        ...
                        <td><input type="number" class="stop-{{$applicant->id}}" name="titan_release" @if(isset($trialarchery) && $trialarchery->titan_release != '') readonly @endif  value="@if(isset($trialarchery)){{$trialarchery->titan_release}}@endif"  id="titan_release{{$applicant->id}}"  oninput="archeryskillTotal({{$applicant->id}})"  style="
                            ...
                            <td><input type="number" class="stop-{{$applicant->id}}"  name="after_hold" @if(isset($trialarchery) && $trialarchery->after_hold != '') readonly @endif  value="@if(isset($trialarchery)){{$trialarchery->after_hold}}@endif"  id="after_hold{{$applicant->id}}"  oninput="archeryskillTotal({{$applicant->id}})"  style="
                                width: 50px;
                                font-weight: 400;
                                background: none;
                                border: 1px solid #cdcdcd;
					...
					<td><input type="number"  class="stop-{{$applicant->id}}"  name="sport_test_mark" readonly @if(isset($trialarchery) && $trialarchery->sport_test_mark != '') value="{{$trialarchery->sport_test_mark}}" @endif  id="sport_test_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="number"  class="stop-{{$applicant->id}}" name="total_obtain_mark" readonly @if(isset($trialarchery) && $trialarchery->total_obtain_mark != '') value="{{$trialarchery->total_obtain_mark}}" @endif  id="total_obtain_mark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 2px solid #868686;
					...
					<td><input type="text" name="remark" class="stop-{{$applicant->id}}" @if(isset($trialarchery))  value="{{$trialarchery->remark}}" @endif  id="remark{{$applicant->id}}" style="
					width: 50px;
					font-weight: 400;
					background: none;
					border: 1px solid #cdcdcd;
...
$(document).ready(function() {
    $('.stop').each(function () {
        var physVal = parseFloat($(this).val());
        if($(this).val() !== '' && physVal < 20){
            var id = $(this).data('id');
            $(`.stop-${id}`).not('[type="checkbox"]').prop('readonly', true);
            $(`.stop-${id}`).filter('[type="checkbox"]').prop('disabled', true);
        }
    });
});
    // All sub sports pre-loaded from server — no AJAX needed
    var allSubSportsData = @json($allSubSports);

    // Gender mapping replicating get_gender() controller logic
    var sportGenderMap = {
        6:3, 10:3, 43:3, 44:3, 20:3, 23:3, 14:3, 25:3, 42:3, 8:3, 9:3, 15:3,
        3:1, 7:1, 4:1,
        5:4
    };
	  ...
	  function sportgender(sport){
		...
		var subsports_id = parseInt($("#sport").attr('data-subsport'));
		var gender = parseInt($("#sport").attr('data-gender'));
        var assign_skill_id = parseInt($("#sport").attr('data-assignskill'));

        var subTypes = allSubSportsData.filter(function(s){ return s.sport_id == sport; });

        if (subTypes.length > 0) {
            $('#subsport_div').show();
            $('select[name="subsport"]').empty().append('<option value="">Select Sub Sport</option>');
            // Populate skill dropdown with same options
            $('select[name="assign_skill"]').empty().append('<option value="">-- Select Skill --</option>');
            $.each(subTypes, function(key, value) {
                ...
                $('select[name="assign_skill"]').append(
                    `<option ${value.id == assign_skill_id ? 'selected' : ''} value="${value.id}"> ${value.sub_type} </option>`);
            });
            // Show/hide skill_div based on whether a sub-sport is currently selected
            updateSkillDivVisibility();
        } else {
            $('#subsport_div').hide();
            $('#skill_div').hide();
            $('select[name="subsport"]').empty();
            $('select[name="assign_skill"]').empty();
        }

        var genderCode = sportGenderMap[sport] || '';

        if (genderCode == 3) {
            $('select[name="gender"]').empty().append('<option value="">Select Gender</option>');
            $('select[name="gender"]').append(`<option value="3" ${gender == 3 ? 'selected' : ''}>Both</option>`);
        } else if (genderCode == 1) {
            $('select[name="gender"]').empty().append('<option value="">Select Gender</option>');
            $('select[name="gender"]').append(`<option value="1" ${gender == 1 ? 'selected' : ''}>Male</option>`);
        } else if (genderCode == 2) {
            $('select[name="gender"]').empty().append('<option value="">Select Gender</option>');
            $('select[name="gender"]').append(`<option value="2" ${gender == 2 ? 'selected' : ''}>Female</option>`);
    ...
    function updateSkillDivVisibility() {
        var subsportVal = $('select[name="subsport"]').val();
        if (subsportVal) {
            // Sub-sport selected for filtering → skill not needed
            $('#skill_div').hide();
            $('select[name="assign_skill"]').val('');
        } else {
            // No sub-sport filter → show skill field for data entry assignment
            var subTypes = allSubSportsData.filter(function(s){ return s.sport_id == parseInt($("#sport").val()); });
            if (subTypes.length > 0) {
                $('#skill_div').show();
            } else {
                $('#skill_div').hide();
    ...
    $(document).ready(function() {
        var initial_sport = $("#sport").val();
        if (initial_sport) {
            sportgender(initial_sport);
        }
        // When sub-sport selection changes, toggle the skill field
        $(document).on('change', 'select[name="subsport"]', function() {
            updateSkillDivVisibility();
        });
    });
	...
	$(`.badminData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#high_double_service_mark${res.application_no}`).prop('readonly', true);
	                $(`#smash_mark${res.application_no}`).prop('readonly', true);
	                $(`#drop_mark${res.application_no}`).prop('readonly', true);
	                $(`#backhand_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkbad${res.application_no}`).prop('disabled', true);
	...
	$(`.volleyballData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#under_hand_mark${res.application_no}`).prop('readonly', true);
	                $(`#upper_hand_mark${res.application_no}`).prop('readonly', true);
	                $(`#service_mark${res.application_no}`).prop('readonly', true);
	                $(`#smash_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkvolleyball${res.application_no}`).prop('disabled', true);
	...
	$(`.basketballData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#dribbling${res.application_no}`).prop('readonly', true);
	                $(`#passing${res.application_no}`).prop('readonly', true);
	                $(`#standing${res.application_no}`).prop('readonly', true);
	                $(`#jumpshot${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkbasketball${res.application_no}`).prop('disabled', true);
	...
	$(`.kustiData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#ground_position_mark${res.application_no}`).prop('readonly', true);
	                $(`#front_position_back_position_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkkusti${res.application_no}`).prop('disabled', true);
	...
	$(`.boxingData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#punching_pad${res.application_no}`).prop('readonly', true);
	                $(`#shadow_boxing${res.application_no}`).prop('readonly', true);
                    $(`#skypink${res.application_no}`).prop('readonly', true);
	                $(`#sparring${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkboxing${res.application_no}`).prop('disabled', true);
	...
	$(`.Data`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#check${res.application_no}`).prop('disabled', true);
	...
	$(`.judoData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#straight_work_throw_mark${res.application_no}`).prop('readonly', true);
	                $(`#hip_leg_hand_techniquec_mark${res.application_no}`).prop('readonly', true);
	                $(`#throw_count_mark${res.application_no}`).prop('readonly', true);
	                $(`#throw_combination_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkjudo${res.application_no}`).prop('disabled', true);
	...
	$(`.archeryData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#staines${res.application_no}`).prop('readonly', true);
	                $(`#knocking${res.application_no}`).prop('readonly', true);
	                $(`#expansion${res.application_no}`).prop('readonly', true);
	                $(`#driving${res.application_no}`).prop('readonly', true);
	                $(`#anchoring${res.application_no}`).prop('readonly', true);
					$(`#titan_hold${res.application_no}`).prop('readonly', true);
                    $(`#aiming${res.application_no}`).prop('readonly', true);
	                $(`#titan_release${res.application_no}`).prop('readonly', true);
					$(`#titan_hold${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
	                $(`#remark${res.application_no}`).prop('readonly', true);

					$(`#checkarchery${res.application_no}`).prop('disabled', true);
	...
	$(`.footballkeeperData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#grip_mark${res.application_no}`).prop('readonly', true);
	                $(`#dive_mark${res.application_no}`).prop('readonly', true);
	                $(`#patch_mark${res.application_no}`).prop('readonly', true);
	                $(`#kick_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkfootballkeeper${res.application_no}`).prop('disabled', true);
	...
	$(`.footballData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#kick_mark${res.application_no}`).prop('readonly', true);
	                $(`#dribble_tackle_mark${res.application_no}`).prop('readonly', true);
	                $(`#head_mark${res.application_no}`).prop('readonly', true);
	                $(`#control_pad_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkfootball${res.application_no}`).prop('disabled', true);
	...
	$(`.hockeyData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#hit_mark${res.application_no}`).prop('readonly', true);
	                $(`#push_mark${res.application_no}`).prop('readonly', true);
	                $(`#dribbling_mark${res.application_no}`).prop('readonly', true);
	                $(`#scoop_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkhockey${res.application_no}`).prop('disabled', true);
	...
	$(`.kabaddiData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#raid_mark${res.application_no}`).prop('readonly', true);
	                $(`#kick_skill_mark${res.application_no}`).prop('readonly', true);
	                $(`#covering_mark${res.application_no}`).prop('readonly', true);
	                $(`#pakad_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkkabaddi${res.application_no}`).prop('disabled', true);
	...
	$(`.tabletennisData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#counter${res.application_no}`).prop('readonly', true);
	                $(`#push${res.application_no}`).prop('readonly', true);
	                $(`#block${res.application_no}`).prop('readonly', true);
	                $(`#service${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checktabletennis${res.application_no}`).prop('disabled', true);
	...
	$(`.hockeykeeperData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#kick_mark${res.application_no}`).prop('readonly', true);
					$(`#pad_mark${res.application_no}`).prop('readonly', true);
	                $(`#high_push_mark${res.application_no}`).prop('readonly', true);
	                $(`#stop_mark${res.application_no}`).prop('readonly', true);
	                $(`#himmat_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkhockeykeeper${res.application_no}`).prop('disabled', true);
...
$(`.swimmingData`).submit(function (e) {
		...
		success: function (res) {
			if (res.error == false) {
				...
				$(`#free_stroke_mark${res.application_no}`).prop('readonly', true);
				$(`#back_stroke_mark${res.application_no}`).prop('readonly', true);
				$(`#breast_stroke_mark${res.application_no}`).prop('readonly', true);
				$(`#butter_fly_mark${res.application_no}`).prop('readonly', true);
				$(`#glaiding_mark${res.application_no}`).prop('readonly', true);
				$(`#start_mark${res.application_no}`).prop('readonly', true);
				$(`#himmat_mark${res.application_no}`).prop('readonly', true);
				$(`#test_score_mark${res.application_no}`).prop('readonly', true);
				$(`#game_technique${res.application_no}`).prop('readonly', true);
				$(`#sport_test_mark${res.application_no}`).prop('readonly', true);
				...
				$(`#checkswimming${res.application_no}`).prop('disabled', true);
	...
	$(`.athleticsthrowerData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#stance_mark${res.application_no}`).prop('readonly', true);
	                $(`#execution_mark${res.application_no}`).prop('readonly', true);
	                $(`#action_mark${res.application_no}`).prop('readonly', true);
	                $(`#follow_throw_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkathleticsthrower${res.application_no}`).prop('disabled', true);
	...
	$(`.gymnasticgirlsData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#balancing_beam_mark${res.application_no}`).prop('readonly', true);
	                $(`#uneven_bar_mark${res.application_no}`).prop('readonly', true);
	                $(`#floor_exercise_mark${res.application_no}`).prop('readonly', true);
	                $(`#vaulving_horse_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkgymnasticgirls${res.application_no}`).prop('disabled', true);
	...
	$(`.gymnasticboysData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#floor_exercise_mark${res.application_no}`).prop('readonly', true);
	                $(`#pommel_horse_mark${res.application_no}`).prop('readonly', true);
	                $(`#ring_mark${res.application_no}`).prop('readonly', true);
	                $(`#vaulving_horse_mark${res.application_no}`).prop('readonly', true);
                    $(`#parallel_bar_mark${res.application_no}`).prop('readonly', true);
                    $(`#horizontal_bar_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkgymnasticboys${res.application_no}`).prop('disabled', true);
	...
	$(`.athleticsjumperData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#approach_mark${res.application_no}`).prop('readonly', true);
	                $(`#t_a_mark${res.application_no}`).prop('readonly', true);
	                $(`#action_mark${res.application_no}`).prop('readonly', true);
	                $(`#landing_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkathleticsjumper${res.application_no}`).prop('disabled', true);
	...
	$(`.cricketbatsmanData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#grip_stance_backlift_mark${res.application_no}`).prop('readonly', true);
	                $(`#ball_select_mark${res.application_no}`).prop('readonly', true);
	                $(`#front_foot_back_foot_mark${res.application_no}`).prop('readonly', true);
	                $(`#front_foot_back_foot_drive_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkcricketbatsman${res.application_no}`).prop('disabled', true);
	...
	$(`.cricketkeeperData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#stumping_mark${res.application_no}`).prop('readonly', true);
	                $(`#gathering_mark${res.application_no}`).prop('readonly', true);
	                $(`#off_stumping_gathering_mark${res.application_no}`).prop('readonly', true);
	                $(`#on_stumping_gathering_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkcricketkeeper${res.application_no}`).prop('disabled', true);
	...
	$(`.cricketballerData`).submit(function (e) {
            ...
            success: function (res) {
                if (res.error == false) {
					...
					$(`#runup_action_followthrough_mark${res.application_no}`).prop('readonly', true);
	                $(`#swing_spin_mark${res.application_no}`).prop('readonly', true);
	                $(`#line_length_mark${res.application_no}`).prop('readonly', true);
	                $(`#speed_flight_mark${res.application_no}`).prop('readonly', true);
	                $(`#test_score_mark${res.application_no}`).prop('readonly', true);
					$(`#game_technique${res.application_no}`).prop('readonly', true);
	                $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
	                $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
					...
					$(`#checkcricketballer${res.application_no}`).prop('disabled', true);
...
$(`.handballData`).submit(function (e) {
    ...
    success: function (res) {
        if (res.error == false) {
            ...
            $(`#dibbling${res.application_no}`).prop('readonly', true);
            $(`#standing_shot${res.application_no}`).prop('readonly', true);
            $(`#jumpshot${res.application_no}`).prop('readonly', true);
            $(`#catch_pass${res.application_no}`).prop('readonly', true);
            $(`#test_score_mark${res.application_no}`).prop('readonly', true);
            $(`#game_technique${res.application_no}`).prop('readonly', true);
            $(`#sport_test_mark${res.application_no}`).prop('readonly', true);
            $(`#total_obtain_mark${res.application_no}`).prop('readonly', true);
            ...
            $(`#checkhandball${res.application_no}`).prop('disabled', true);
    ...
    $.ajax({
            ...
            success: function (res) {
                if (res.error == false) {
					...
					success(res.msg);
					$(`#hund${res.application_no}`).prop('readonly', true);
	                $(`#eight${res.application_no}`).prop('readonly', true);
	                $(`#jump${res.application_no}`).prop('readonly', true);
	                $(`#shuttle${res.application_no}`).prop('readonly', true);
	                $(`#ball${res.application_no}`).prop('readonly', true);
					$(`#hundtime${res.application_no}`).prop('readonly', true);
	                $(`#eighttime${res.application_no}`).prop('readonly', true);
	                $(`#jumpdist${res.application_no}`).prop('readonly', true);
	                $(`#shuttletime${res.application_no}`).prop('readonly', true);
	                $(`#balldist${res.application_no}`).prop('readonly', true);
					$(`#check${res.application_no}`).prop('disabled', true);


                    if(parseFloat($(`#phy${res.application_no}`).val()) >= 20){
                        $(`.stop-${res.application_no}`).not('[type="checkbox"]').prop('readonly', false);
                        $(`.stop-${res.application_no}`).filter('[type="checkbox"]').prop('disabled', false);
                    } else {
                        $(`.stop-${res.application_no}`).not('[type="checkbox"]').prop('readonly', true);
                        $(`.stop-${res.application_no}`).filter('[type="checkbox"]').prop('disabled', true);
                    }
...
$(document).ajaxError(function(event, jqXHR, ajaxSettings, thrownError) {
    if (jqXHR.status === 419) {
        ...
        try {
            ...
            if (resp.errors) {
                var msgs = [];
                $.each(resp.errors, function(k, v) { msgs.push(v[0]); });
                error(msgs.join(' | '));
            } else {
                error(resp.message || 'Validation failed.');
            }
        } catch(e) {
        ...
        error('Request failed (' + jqXHR.status + '). Please try again.');
    }
});
...
if (!toPrint) {
    error('Please filter data first to enable printing.');
    return;
}
