...
use App\Models\HostelSportModel;
use App\Models\SportModel;
use Illuminate\Support\Facades\Auth;

class HostelSportsController extends Controller
{
    public function index()
    {
        $sports = DB::table('sport_master')->orderBy('id', 'DESC')->get();
        return view('admin.division_master.hostel-sport-list', ['sports' => $sports]);
    }
    ...
    public function saveSport(Request $req)
    {
        ...
        $req->validate([
            'name'     => 'required|max:150|unique:sport_master',
            'status'      => 'required',
            'gender'      => 'required',
    ...
    {
        $divStatus = DB::table('sport_master')->where('id', $id)->first()->status;
        DB::table('sport_master')->where('id', $id)->update(["status" => $divStatus == 1 ? 0 : 1]);
     ...
     public function updateSport(Request $req)
     {
        ...
        $validation = Validator::make($req->all(), [
            ...
            'id'           => 'required|max:10',
            'gender'           => 'required',
            'trial_female_date'           => 'required',
            'trial_male_date'           => 'required',
        ...
        DB::table('sport_master')->where('id', $req->id)->update(["name" => $req->name, "ip_address" => $req->ip(),"gender"=>$req->gender, "current_user_id" => Auth::guard('admin')->user()->id,"trial_male_date"=>$req->trial_male_date, "trial_female_date"=>$req->trial_female_date]);
