...
class FacilityBookingController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if ($request->method() == 'POST') {
            ...
            if (Auth::guard('facility_booking')->attempt(['email' => $request->email, 'password' => $request->password])) {
    ...
    public function otp(Request $request)
    {
        ...
        if ($request->method() == 'POST') {
            ...
            if ($user) {
                $user->otp_verify = 1;
                $user->save();
                SmsMail::dispatch(["id" => $user->id], 22);
    ...
    public function change_password(Request $request)
    {
        ...
        if ($request->method() == 'POST') {
            ...
            $data = [
                'decoded_password' => $request->confirm_password,
    ...
    public function application(Request $request)
    {
        ...
        $validation = Validator::make($request->all(), [
            'organisation_document' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_photo' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_aadhaar' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_parent_aadhaar' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_parent_signature' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_age_proof' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'mbbs_doctor_certificate' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'address_proof' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',

            'applicant_photo_gr' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_aadhaar_gr' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_signature_gr' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_photo_spm' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_signature_spm' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'mbbs_doctor_certificate_spm' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',

            'address_proof_spm' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_photo_spa' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_aadhaar_spa' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_signature_spa' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',

            'applicant_photo_s' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_aadhaar_s' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
            'applicant_signature_s' => 'nullable|mimes:pdf,png,jpg,jpeg|max:2000',
        ...
        if ($request->method() == 'POST') {
            ...
            $dataApplication = [
                ...
                'city' => $request->city,
                'pin' => $request->pin,
                'organisation_fee_exemption' => $request->organisation_fee_exemption,
    ...
    public function application_update(Request $request, $id)
    {
        ...
        $validation = Validator::make($request->all(), [
            'organisation_document' => 'nullable|mimes:png,jpg,jpeg,pdf|max:2048',
            'applicant_photo_spa' => 'nullable|mimes:png,jpg,jpeg,pdf|max:2048',
            'applicant_aadhaar_spa' => 'nullable|mimes:png,jpg,jpeg,pdf|max:2048',
            'applicant_signature_spa' => 'nullable|mimes:png,jpg,jpeg,pdf|max:2048',
        ], msg());
    ...
    public function final_submit($id)
    {
        ...
        if (DB::table('facility_booking_type_detail')->where('id', $id)->where('query_status', 1)->first()) {
            ...
            return response()->json(['error' => false, 'msg' => 'Application Re-Submitted successfully.', 'url' => route(
                'facility_booking_application_preview',
                $id
            )]);
        } else {
            DB::table('facility_booking_type_detail')->where('id', $id)->update([
            ...
            return response()->json(['error' => false, 'msg' => 'Application final Submitted successfully.', 'url' => route(
                'facility_booking_application_preview',
                $id
            )]);
        }
    ...
    {
        $count = FacilityRegister::where('email', session()->get('email'))->first()->otp_count;
    ...
    public function admin_dashboard($type)
    {
        ...
        $application =  DB::table('facility_booking_register')->leftJoin('facility_booking_type_detail', 'facility_booking_type_detail.user_id', '=', 'facility_booking_register.id')
            ->leftJoin('facility_booking_service_detail', 'facility_booking_service_detail.application_id', '=', 'facility_booking_type_detail.id')
            ->select(
                'facility_booking_type_detail.id as id_application',
                'accept_reject_date',
                'query_status',
                'final_submit_date',
                'marked_on',
                'facility_booking_type_detail.application_no',
                'facility_booking_type_detail.service',
                'facility_booking_type_detail.booking_type',
                'facility_booking_register.name',
                'facility_booking_register.email',
                'facility_booking_register.mobile',
                'facility_booking_type_detail.status',
                'facility_booking_type_detail.final_submit',
                'facility_booking_type_detail.payment_date',
                'facility_booking_type_detail.amount_to_be_paid',
                'facility_booking_type_detail.payment_status',
                'facility_booking_type_detail.fee_exemption_status'
            )
            ->where('service', $type)->where('facility_booking_type_detail.final_submit', 1);
        ...
        // dd($application);
        return   view('facility_booking.admin_dashbboard', compact('application', 'type'));
    }

    public function  facilityBookingAjax(Request $req)
    {
        ...
        $query =  DB::table('facility_booking_register')->leftJoin('facility_booking_type_detail', 'facility_booking_type_detail.user_id', '=', 'facility_booking_register.id')
            ->leftJoin('facility_booking_service_detail', 'facility_booking_service_detail.application_id', '=', 'facility_booking_type_detail.id')
            ->select(
                'facility_booking_type_detail.id as id_application',
                'accept_reject_date',
                ...
                'facility_booking_type_detail.payment_status',
                'facility_booking_type_detail.fee_exemption_status'
            )->where('facility_booking_type_detail.final_submit', 1);
        ...
        if ($req->search != '')
            $query->whereRaw(" ( facility_booking_type_detail.application_no LIKE '%$req->search%' OR facility_booking_type_detail.booking_type LIKE '%$req->search%' OR facility_booking_register.name LIKE '%$req->search%'
            OR facility_booking_register.email LIKE '%$req->search%' OR facility_booking_register.mobile LIKE '%$req->search%') ");


        if (Auth::guard('admin')->user()->division_id) {
            $query->whereIn('facility_booking_service_detail.location', explode(',', $check[0]->district_id));
        }
        if (Auth::guard('admin')->user()->district_id) {
            $query->where('facility_booking_service_detail.location', explode(',', Auth::guard('admin')->user()->district_id));
        }

        if ($req->type != '') {
            $query->where('service', $req->type);
        }
        ...
        $vaaa = $query;
        $recordsTotal = count($vaaa->get());

        // if (isset($req->order[0]['dir'])) {
        // 	$orderBy = $req->order[0]['dir'] ?? 'desc';
        // 	$orderByName = $req->order[0]['name'];
        // 	$query->orderBy($orderByName, $orderBy);
        // }
        $query->orderBy('final_submit_date', 'desc');

        $collection = $query->skip($skip)->take($pageLength)->get();
        return response()->json(["draw" => $req->draw, "recordsTotal" => $recordsTotal, "recordsFiltered" => $recordsTotal, 'data' => $collection], 200);
    }
    ...
    public function accepted_reject_status(Request $request)
    {
        ...
        if ($request->status == 1) {
            ...
            return response()->json(['error' => false, 'msg' => 'Application Accepted Successfully', 'url' => route(
                'admin_facility_booking_preview',
            ...
            return response()->json(['error' => false, 'msg' => 'Application Rejected Successfully', 'url' => route(
                'admin_facility_booking_preview',
                $request->application_id
            )]);
        }
