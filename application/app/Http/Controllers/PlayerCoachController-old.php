...
class PlayerCoachController extends Controller
{
    ...
    public function basicDetailStore(Request $request){
         ...
         if ($validation->fails())
            ...
            $user_basic = DB::table('player_coach_basic_detail')->where('user_id',Auth::guard('player')->user()->id)->first();
             ...
             if ($request->hasFile('profile_picture')){
                $profile_picture = moveFile('player_coach/profile_picture', $request->profile_picture);
                $data['profile_picture']= $profile_picture;

            }

            if ($request->hasFile('signature')){
                $signature = moveFile('player_coach/signature', $request->signature);
       ...
       if($user_basic){
        ...
        $profile_picturee = "public/player_coach/profile_picture/".$user_basic->profile_picture;


       if($request->hasFile('profile_picture') && File::exists($profile_picturee)) {
            File::delete($profile_picturee);
        }
        $signaturee = "public/player_coach/signature/".$user_basic->signature;
