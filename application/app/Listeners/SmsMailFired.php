...
class SmsMailFired
{
    ...
    public function handle(SmsMail $event)
    {
        if ($event->type == 1) {
            ...
            $message = 'Dear ' . $users->fullname . ', Your Khel Sathi Login Details: User ID: ' . $users->email . ' Password: ' . $user->user_password . ' -Khel Sathi Portal';

            loginCredential($user->mobile, $message);
            try {
            ...
            $message = 'Dear ' . $users->name . ', Your Khel Sathi Login Details: User ID: ' . $users->email . ' Password: ' . $users->decoded_password . ' -Khel Sathi Portal';

            loginCredential($users->mobile, $message);
            try {
            ...
            if (isset($event->item['otp'])) {
                $otp = $event->item['otp'];
                // dd($event->item['email']);
                $message = 'Your OTP for Sports College Admission Registration is ' . $otp . '. Do not share with anyone. -Khel Sathi';
                SendSMS($event->item['mobile'], $message);
                ...
                $users = DB::table('sport_welfare_registration_master')->select('fullname', 'email', 'user_password')->where('email', $event->item['email'])->first();
                    ...
                    Mail::to($event->item['email'])->send(new \App\Mail\ForgetPassword($users->fullname, $users->email, $users->user_password));
                ...
                $message = 'Dear ' . $users->fullname . ', Your Khel Sathi Login Details: User ID: ' . $users->application_no . ' Password: ' . $users->user_password . ' -Khel Sathi Portal';
            ...
            elseif ($event->type == 21) {
                $users = PrivateCoaching::where('id', $event->item['id'])->first();
            ...
            elseif ($event->type == 23) {
                $password = $event->item['password'];
                $email = $event->item['email'];
                $users = DB::table('private_coaching_register')->select('name', 'email','decoded_password' , 'mobile')->where('email', $event->item['email'])->first();
            ...
            elseif ($event->type == 24) {
                $email = $event->item['email'];
                $users = DB::table('eklavya_fund_registration')->select('academy_name', 'email','decoded_password' , 'mobile')->where('email', $event->item['email'])->first();
                $message = 'Dear ' . $users->academy_name . ', Your Khel Sathi Login Details: User ID: ' . $users->email . ' Password: ' . $users->decoded_password . ' -Khel Sathi Portal';
