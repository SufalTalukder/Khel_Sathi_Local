...
class FinancialAssistance_Model extends Model
{
    ...
    static function preRegistration($req)
    {
        ...
        try{
            ...
            $income_certificate = date('His').$req->income_certificate->getClientOriginalName();
            ...
            $relevant_certificate = date('His').$req->relevant_certificate->getClientOriginalName();
        ...
        if($req->document_other_achievements){
            $document_other_achievements = date('His').$req->document_other_achievements->getClientOriginalName();
            ...
            $income_document_other_sources = date('His').$req->income_document_other_sources->getClientOriginalName();
            ...
            $document_justifying_experience = date('His').$req->document_justifying_experience->getClientOriginalName();
            ...
            if($req->relevant_documents_justifing_assistance){
                $relevant_documents_justifing_assistance = date('His').$req->relevant_documents_justifing_assistance->getClientOriginalName();
        ...
        if($req->medical_certificate){
            ...
            $user = DB::table('financial_assistance')->where('user_id', Auth::id())->where('form_status','!=',2)->orderBy('id','desc')->first();
            if($user){

                static::update_updatefinancialform($req);
                return redirect('financialformPreview/'.$user->application_no)->with('success','Successfully Updated!!');
            }
                    ...
                    // dd($applicationNo);
                    $check = DB::table('financial_assistance')->insertGetId($data);
                        ...
                        return response()->json(["error" => false, "msg" => 'Submitted Successfully. !!', "url" => url('financialformPreview/'.$applicationNo)]);
    ...
    static function update_updatefinancialform($req)
    {
       ...
       if(empty($applicationNo)){
        $user = DB::table('financial_assistance')->where('user_id', Auth::id())->where('form_status','!=',2)->orderBy('id','desc')->first();
        $applicationNo= $user->application_no;
       }
        ...
        try{
            ...
            //     print_r($sport_achievement_docs);exit;
                ...
                $check = DB::table('financial_assistance')->where('user_id', Auth::id())->where('application_no', $applicationNo)->update([
                    ...
                    'other_relevant_information_applicant' =>$req->other_relevant_information_applicant,
                        ...
                        foreach($req->sport_achievement as $key=>$item){
                         ...
                         return response()->json(["error" => false, "msg" => 'Updated Successfully. !!', "url" => url('financialformPreview/'.$applicationNo)]);
    ...
    static function save_monthlyPension($req)
    {
        ...
        try{
            ...
            if($user){
                ...
                return redirect('monthlypensionformpreview')->with('success','Successfully Updated!!');
            }

                else{
            $award_certificate = date('His').$req->award_certificate->getClientOriginalName();
            $path = $req->file('award_certificate')->storeAs('financial_assistance', $award_certificate, 'public');

            $domicile_certificate = date('His').$req->domicile_certificate->getClientOriginalName();
            $path = $req->file('domicile_certificate')->storeAs('domicile_certificate', $domicile_certificate, 'public');
                ...
                $check = DB::table('monthly_pension')->insertGetId(array(
                   ...
                   'award_year' =>$req->award_year,
                    ...
                    // 'guardian_signature' =>$guardian_signature,
                        ...
                        return response()->json(["error" => false, "msg" => 'Submitted Successfully. !!', "url" => url('monthlypensionformpreview/'.$applicationNo)]);
...
return response()->json(["error" => true, "msg" => $e->getMessage()]);
    ...
    static function updatemonthlypensionform($req)
    {
        ...
        return response()->json(["error" => true, "msg" => $validation->errors()->first()]);
            ...
            //  return redirect('/financial-assistance/edit_monthlypensionform')->withInput($req->all())->with('error', $validation->errors()->first());
        ...
        try{
            ...
            // $path = $req->file('award_certificate')->storeAs('financial_assistance', $award_certificate, 'public');
                ...
                $check = DB::table('monthly_pension')->where('user_id', Auth::id())->where('application_no', $req->application_no)->update([
                    // 'user_id' =>Auth::id(),
                    'sport_type' =>$req->sport_type,
                    'award_certificate' =>$award_certificate,
                    'domicile_certificate' =>$domicile_certificate,
                    ...
                    return response()->json(["error" => false, "msg" => 'Submitted Updated. !!', "url" => url('monthlypensionformpreview/'.$application_no)]);
